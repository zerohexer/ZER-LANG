 ...
 * Either explicitly named "free" OR bodyless void fn with *opaque or *T first
...
static bool ir_is_extern_free_call(ZerCheck *zc, Node *call) {
    ...
    /* Signature heuristic: bodyless fn taking *opaque or *T ...
...
bool zercheck_ir(ZerCheck *zc, IRFunc *func) {
    ...
    if (getenv("IR_SUMMARY_DEBUG") && func->ast_node &&
        ...
        fprintf(stderr, "ZCIR: building=%d fn='%.*s' pc=%d blocks=%d sumcount=%d\n",
            zc->building_summary,
            /* %.*s takes an int; passing a size_t through varargs is UB (it
             * happens to work on x86-64 because the callee reads the low half
