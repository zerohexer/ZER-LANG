...
30. **Array init/assignment → `memcpy`** — C arrays aren't lvalues. `u32[4] b = a` and `b = a` produce invalid C. Emitter detects TYPE_ARRAY in var-decl init (emit `= {0}` + memcpy) and NODE_ASSIGN (emit `memcpy(target, value, sizeof(target))` + goto assign_done).
