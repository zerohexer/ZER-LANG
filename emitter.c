...
static Node *emit_defer_shared_root(Emitter *e, Node *expr) {
    ...
    if (expr->kind == NODE_BINARY) {
        ...
        /* AUDIT-2026-07-01 form-coverage: defer body reads a shared struct
         * through an intrinsic (`@truncate(u32, g.v)`, `@ptrcast(*T, &g.f)`)
         * must still emit the mutex lock at defer-fire — the walker previously
         * returned NULL on NODE_INTRINSIC, so the deferred access was emitted
         * unlocked = silent race at defer fire. Mirror of the primary
         * find_shared_root_expr (ir_lower.c ~1265). Condvar/barrier/once
         * intrinsics handle their own lock — don't double-wrap. */
        ...
        /* AUDIT-2026-07-01 form-coverage: `defer { x = a() orelse g.v; }`. */
        ...
        /* AUDIT-2026-07-01 form-coverage: `defer { s = g.buf[0..n]; }`. */
        ...
        /* AUDIT-2026-07-01 form-coverage: `defer { P p = { .x = g.v }; }`. */
...
static void emit_defer_stmt(Emitter *e, Node *s, IRFunc *func) {
    ...
    case NODE_FOR:
        ...
        return;
    case NODE_VAR_DECL: {
        /* AUDIT-2026-07-01 form-coverage: lock-wrap deferred shared-struct
         * reads in a var-decl init too, not only in NODE_EXPR_STMT above.
         * `defer { u32 z = g.v; }` (or the intrinsic-wrapped form) must
         * emit a rdlock, else the deferred read at fire-time races. Read
         * lock (var-decl is a read); recursive mutex makes it safe if a
         * lock is already held (BUG-473). */
