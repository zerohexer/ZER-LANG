...
| "which entry does this projection name?" — a `hp.p` through `*H hp = &h` IS `h.p` | `ir_extract_compound_key(zc, func, ps, expr, …)` — now takes the PATH STATE and re-roots through `IRHandleInfo.view_root_local` (BUG-999); a GLOBAL projection resolves to `(IR_GLOBAL_ROOT_ID, "g.p")` via `ir_global_projection_key` (BUG-1001) | every one of the ~36 zercheck_ir sinks. Never build a compound key by hand |
| "an allocation stored straight into a SLOT" (`h.p = alloc(T)`, `arr[0] = alloc(T)`, `g.p = alloc(T)`) | `ir_register_alloc_result_compound` — the slot sibling of `ir_register_alloc_result` (BUG-1000) | the passthrough IR_ASSIGN alloc arm |
...
| "which SLOT does `hs[k].p` name?" — the symbolic-index key discipline (BUG-1012) | `ir_extract_compound_key` mode 1 keys an INDEX-STABLE local (`ir_local_index_stable`: never reassigned/address-taken, one definition outside any CFG cycle, uniquely named) as `"[k].p"`; `ir_extract_compound_key_ex(…, 2)` keys anything else as the WILD `"[?].p"`; `ir_key_may_alias(a, b)` says whether two keys may name one slot (two DIFFERENT literals never do; a symbolic/wild step always may) | every zercheck_ir sink for free. A projection with no exact entry but an index step is read as a VIEW of its may-alias siblings (`ir_view_root_local` → `ir_unkeyable_load_view` / `ir_unkeyable_read_check`); a store into a wild slot adds to the ROOT's view set (`ir_unkeyable_store_view`); a free through any spelling widens the aliasing siblings (`ir_index_free_siblings`, from `ir_free_widens_index_aliases` on the freed alloc_id and `ir_variable_index_free` for an unkeyable argument) and flags them `idx_widened` — a WILD read skips an index-widened member (`ir_view_pull_check`, the ONE pull), a KEYED read does not. Never add a free sink without the widening, never add a load arm without the view |
| "does this counted loop's counter WILL-hold every value of its sequence?" (a VarRange is only MAY-hold) | `Checker.cert_loop_*` set by the `for` / `while` drivers (`loop_body_straight_line`, `while_counter_step`, `loop_seq_reaches_limit`, BUG-1011) | the fixed-array index sink: counter matches, access at the body's own `branch_depth`, sequence reaches the size → ERROR. Never turn a MAY-hold `IDX_PARTIAL_OOB` into an error without this fact |
