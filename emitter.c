...
/* BUG-1003: render a `double` as a VALID C floating constant.
 *
...
static CallArgForm call_arg_form(Emitter *e, Type *callee_type, Node *arg, int i,
    ...
    if (!arg_type || type_dispatch_kind(eff_callee) != TYPE_FUNC_PTR ||
        (uint32_t)i >= eff_callee->func_ptr.param_count)
        return CARG_PLAIN;
    Type *param = eff_callee->func_ptr.params[i];
    if (param_out) *param_out = param;
    TypeKind ak = type_dispatch_kind(arg_type), pk = type_dispatch_kind(param);
    if (ak == TYPE_SLICE && pk == TYPE_POINTER) return CARG_DECAY;
    if (ak == TYPE_ARRAY && pk == TYPE_SLICE) return CARG_ARR_TO_SLICE;
    return CARG_PLAIN;
}
...
/* ================================================================
 * CAST EMISSION — ONE POLICY, THREE CALL SITES  (BUG-1000, 2026-09-12)
 * ================================================================
 *
...
/* BUG-1002: the same saturation as a C CONSTANT EXPRESSION — no statement
...
/* BUG-1001 (2026-09-12): a function whose RETURN TYPE is a function pointer needs
...
static void emit_expr(Emitter *e, Node *node) {
    ...
    case NODE_IDENT: {
        /* BUG-997: inside a GLOBAL initializer a name is not a C constant
         ...
         * depth counter doubles as the recursion bound (the checker already
         * refuses a cyclic chain, BUG-975). */
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
             ...
             * BUG-991: was `nlen >= 10`, which excludes `@atomic_or` (9 chars) —
...
static void emit_global_var(Emitter *e, Node *node) {
    ...
    if (node->var_decl.init) {
        ...
        if (gtype_eff && gtype_eff->kind == TYPE_OPTIONAL &&
            ...
            if (!emitted_const) {
                emit(e, " = ");
                /* BUG-997: mark the global-initializer context so a NODE_IDENT
...
static void emit_cast_value(Emitter *e, const CastOperand *op,
    ...
    case CASTF_TO_BOOL:
        ...
        break;
    case CASTF_F2I: {
        /* BUG-1002: the AST operand is the GLOBAL-INITIALIZER path, where a
...
static void emit_rewritten_node(Emitter *e, Node *node, IRFunc *func) {
    ...
    case NODE_CALL: {
        ...
        if (node->call.callee && node->call.callee->kind == NODE_FIELD &&
            ...
            /* ThreadHandle.join() → pthread_join(th, NULL).
             *
             * BUG-1009: the comment here used to say "field name `join` + object
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            /* BUG-991: this is the IR-path TWIN of the AST-path fallback that
...
static void emit_ir_inst(Emitter *e, IRInst *inst, IRFunc *func) {
            ...
            if (inst->func_name) {
                ...
                if (callee->field.object && callee->field.object->kind == NODE_IDENT) {
                    ...
                    if (hot && type_dispatch_kind(hot) == TYPE_HANDLE) {
                        /* BUG-1010: calling a function-pointer FIELD through a
    ...
    case IR_GOTO: {
        emit_indent(e);
        /* BUG-1008: union switch totality guard (1zukjq's BUG-1008). ir_lower.c
...
/* BUG-993 (lzmkhn's BUG-917): re-anchor the C preprocessor's line counter to
...
static void emit_regular_func_from_ir(Emitter *e, IRFunc *func) {
     ...
     * (BUG-418 class). BUG-993 re-anchors it per INSTRUCTION instead, which is
    ...
    for (int bi = 0; bi < func->block_count; bi++) {
        ...
        for (int ii = 0; ii < bb->inst_count; ii++) {
            ...
            /* BUG-993: anchor the line counter BEFORE the guards, so an
...
static void emit_async_func_from_ir(Emitter *e, IRFunc *func) {
    ...
    /* Disable source mapping during IR blocks (see the BUG-993 note on the
