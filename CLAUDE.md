...
| **Value-flow compatibility ("may this value land in this destination?")** | var-decl init, assignment, call arg, return, spawn arg, struct-init field, orelse fallback, global init, **compound-assign RHS, integer switch ARM** (BUG-925 — the arm was the one site that asked NOTHING: `-1 =>` on a u32 matched 4294967295, `true =>` on a u8 compared as 1) | ONE query **`value_flows_to`** (BUG-842) + ONE post-acceptance retype **`retype_accepted_const`** (BUG-922 — it was at four of the eight, so `a = -(1 << 40)` computed in u32 while `i64 a = -(1 << 40)` computed in i64). The three-condition chain `!type_equals && !can_implicit_coerce && !is_literal_compatible` used to be written out at all EIGHT, which is exactly why a negative constant into an unsigned type was accepted at every one of them. Each site keeps its own wording; only the DECISION and the RETYPE are shared. Literal emission is ONE `emit_int_literal` (both AST paths); a cast to uN/iN is wrapped at THREE emitter sites by `emit_intn_cast_wrap_*` |
...
2. **C-style casts ARE supported** for type conversion. `@saturate`, `@bitcast`, and `@pun` remain for the rare cases.
   ...
   (u3)x / (i48)x                 // OK — uN/iN target keeps exactly N bits ((u3)300 == 4)
