...
### Cast EMISSION is one function, not three (BUG-963, 2026-09-02)
...
## Bounds Auto-Guard (checker.c + ir_lower.c) — an IR branch since BUG-994 (2026-09-05)
...
emitter, BUG-995). The emitter has no guard code left; `_zer_bounds_check` on slices is
...
### Defer bodies are LOWERED INTO THE IR at every fire site (2026-09-05, BUG-995) — read this first
...
per-statement shared-lock wrapper, the auto-guards (`lower_auto_guards`, BUG-994), the
...
- **B5 defer-body lock** (BUG-749): originally `emit_defer_stmt` NODE_EXPR_STMT
  ...
  Since BUG-995 (2026-09-05) the defer body is lowered through `lower_stmt`, so the
...
**A LITERAL has no provenance (BUG-985, 2026-08-30).** All six literal kinds classify
...
## Audit 2026-09-04 — the mechanisms behind BUG-946..919 (read before touching any of them)
...
**Capture locals dedup only with capture locals (`ir_add_local`, BUG-946).** The dedup key
...
**Per-statement lock collector covers NODE_SPAWN per ARGUMENT (BUG-947).** The emitter locks
...
**Static locals in the race scans (BUG-948).** Spawn: `scan_unsafe_global_access` keeps a
...
**Unique-resource copies (BUG-949).** `type_unique_resource_name` (Pool/Ring/Slab/Arena/
...
**Enum non-variant guard (BUG-950).** `ir_lower` keeps the elided last-arm entry of an
...
**Zero value of a return type (BUG-951).** `emit_return_null` delegates to `emit_zero_value`,
...
**View-root resolution (BUG-952).** `ir_view_root_handle(zc, func, ps, expr)` in zercheck_ir.c
...
### BUG-953..924 — the second half of the 2026-09-04 audit (mechanisms)

- **Slice header is read-only (BUG-953).** `slice_header_field(c, n)` names `.ptr`/`.len` on a
...
- **Parser lookahead for `IDENT ( *` (BUG-954).** The C-style funcptr declaration is committed
  only on `IDENT ( * IDENT ) (`; anything else is an expression statement. `f(*p);` is a call.
- **Packed array-field views (BUG-955).** `packed_array_field_view(c, v)` = NODE_FIELD whose type
...
- **uN bit-slice store (BUG-956).** `emit_intn_mask` wraps VALUES (IR_BINOP/IR_UNOP); the
...
- **Borrow root (BUG-957) — the scoped-spawn sink's missing variable.** `Symbol.borrow_root_name`
