...
| **Launder peel ACROSS LAYERS (BUG-914)** | `checker.c` `unwrap_ptr_launder`, `zercheck_ir.c` `ir_peel_cast_wrappers` + `ir_find_store_source_local`, and any hand-rolled peel | The two layers ask the SAME question ("what does this value alias?") and drifted: BUG-791 taught the checker about C-style casts and zercheck_ir was never told, so `g = (*N)n` laundered a HEAP pointer while `g = n` and `g = @ptrcast(*N,n)` were both rejected — with byte-identical emitted C, because the emitter elides an identity pointer cast. THE rule now lives in `ast.h` as `zer_tynode_is_reference_producing`; **a new launder form must be added THERE, not at a call site.** Gate: **p19 axis** in `tools/sink_matrix.sh` |
...
| `sink_matrix.sh` | `SINK MATRIX CLEAN` (cell count drifts — RUN it, never cite the number) |
...
| Enum forging (a value outside the variant set) | TWO layers. (1) Runtime variant guard at the point of forgery for the three VALUE conversions: `@bitcast` (BUG-843), `@truncate` (BUG-891), `@saturate` (BUG-910) — these name the culprit, so prefer them. `@cast` is not one (it needs a distinct typedef and cannot name a bare enum). The guard recurses struct fields, optional payloads and array elements, and each wires it at BOTH emitter dispatch paths. (2) A BACKSTOP at the point of CONSEQUENCE (BUG-916): the last arm of an exhaustive enum `switch` COMPARES and traps on a non-variant instead of being entered unconditionally. **The door set was called "closed" three times and was not** — `@ptrcast(*State, &some_u32)` forges through a POINTER, where no door-side guard can reach (the cast yields no value to check). That is why the backstop exists: it covers a route nobody has enumerated. Still wire the guard on any new value-producing conversion; the backstop is the net, not the plan. |
