...
| **Static locals ("is this name global STORAGE?")** | spawn scan (`scan_unsafe_global_access`), ISR recorder (`record_isr_globals`), the check-time NODE_IDENT/NODE_ASSIGN ISR track, `resolve_write_target_global` | ONE push helper `static_alias_note_decl` + the scan-scoped `_static_alias` table (BUG-967); a walker that resolves names through `global_scope` only is BLIND to them — consult the table first |
| **Unique-resource copy ("does copying this VALUE duplicate allocator/lock state?")** | the eight value-flow sinks | `type_carries_unique_resource` asked inside `value_flows_to` (BUG-970) — never a per-sink `kind == TYPE_ARENA` test |
