...
skipped the normalization; see BUGS-FIXED BUG-963.)
...
- The lock is emitted **per statement, in every statement position** — a function
  ...
  form of the same line locked correctly. See BUGS-FIXED BUG-965, and
...
A reader-writer lock is **not re-entrant**. A statement that accesses a
`shared(rw)` struct directly holds its lock for the whole statement, so a
function it calls in that same statement must not touch the same struct
(directly or transitively) — that is a compile error (since 2026-09-06). A
...
u32 main() {
    g.v = f();             // COMPILE ERROR — S locked here and again inside f
...
- A callback the spawn target invokes is scanned too — if it touches a
  ...
  compiled clean; see BUGS-FIXED BUG-961. The identical rule and the identical
