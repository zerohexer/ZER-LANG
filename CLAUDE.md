...
| **Launder peel ("does this wrapper preserve the value's provenance?")** | every escape/free sink + the alloc-key extractor | ONE peeler `unwrap_ptr_launder` + the **p15 and p19 axes in `tools/sink_matrix.sh`**. A `orelse` is a JOIN (two nodes, not one) so it needs the PREDICATE `value_frame_bound_symbol`, not a peel. **The debt was PAID 2026-08-27 (BUG-919) and it was a live hole when paid**: `@container` and `@cstr` return a pointer INTO `args[0]` while every other launder passes it LAST, six sites open-coded the last-arg peel, and `gp = @cstr(localbuf, s)` published a pointer into a dead frame to a global (ASan stack-use-after-return). Five sites now call the peeler; the sixth (`spawn_arg_is_stack_derived`) is an ALLOW-LIST, conservative by construction, and was probed masked. **The peel alone was not enough** — peeling `@cstr` yields a BARE IDENTIFIER naming a local array, a state no shape-matching sink had a case for, so `value_frame_bound_symbol` gained the local-array-decay arm and the two remaining shape-matchers now ASK it rather than growing a case per launder. If you add a launder, that is the ONE place to teach |
...
| Enum forging (a value outside the variant set) | Runtime variant guard at the point of forgery — TRACKED, not banned, so reading an enum from a register still works (as `@bitcast(State, *u32reg)`). The GUARDED doors are EXACTLY THREE: `@bitcast` (BUG-843), `@truncate` (BUG-891), `@saturate` (BUG-910). `@cast` is NOT a door — it requires a distinct typedef and cannot name a bare enum. `@inttoptr` to an enum-carrying pointee WAS a fourth route (BUG-917: `volatile *State reg = @inttoptr(*State, a); switch (*reg)` returned an arm on a register holding 200, zero guards emitted) and is now **REJECTED**, keeping the guarded set at three — guarding it would have meant firing at every READ through the pointer (deref/field/index/nested), a new N-sink surface, and the rejection routes users to the guarded `@bitcast` idiom the docs already teach. The guard recurses struct fields, optional payloads and array elements, and each door wires it at BOTH emitter dispatch paths. **Adding a new value-producing conversion? It is a fourth door — wire the guard. Adding a new way to POINT AT foreign memory? Ask whether an enum can be read through it.** The reason every one of these reports as severe: the exhaustive-switch lowering emits the LAST arm as an unconditional `else`, so a non-variant value *takes an arm* instead of falling through — see docs/limitations.md "the last-arm elision is the amplifier". |
