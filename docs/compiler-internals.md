...
- **Phase 8b continued:** IR_BINOP/IR_UNOP/IR_FIELD_READ/IR_INDEX_READ now emit from local IDs (primary path). emit_expr only as fallback for complex types (handle auto-deref, opaque, pool/slab). BUG-510: param types from `checker_get_type(func_decl)->func_ptr.params[i]` instead of scope_lookup (was ty_void for *T params).
- **Remaining (21 `emit_expr` calls):** IR_ASSIGN (4 — capture type mismatch, ?void hoist, main assign, void expr), IR_CALL (2 — expr + args), IR_BRANCH (2 — condition), IR_RETURN (2 — return expr), IR_AWAIT (1 — condition), builtins (1), IR_INTRINSIC (1), 3AC fallbacks (4 — when src local is -1 or complex type), IR_NOP (1 — INTENTIONAL spawn/asm passthrough). Verification target: `grep emit_expr` in emit_ir_inst = 1 (IR_NOP only).
