...
- **Phase 8c COMPLETE:** All 17 `emit_expr(e,...)` calls in `emit_ir_inst` replaced with `emit_ast_bridge(e,...)`. Bridge function delegates to `emit_expr` but isolates the AST dependency. `grep "emit_expr(e," emit_ir_inst` = 0 (VERIFIED). Future: replace `emit_ast_bridge` calls with local-ID emission per expression type (calls, intrinsics, builtins, orelse — each is ~50 lines of emission logic).
