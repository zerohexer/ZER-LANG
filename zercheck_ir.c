...
typedef struct {
     ...
     * still-alive path). */
    int freed_all_paths;
    /* BUG-988: this SLOT entry was FREED (resp. MAYBE_FREED) and then RESET
    ...
    /* BUG-989: this local is a POINTER VIEW of a whole local AGGREGATE
...
static IRHandleInfo *ir_alloc_handle_slot(IRPathState *ps) {
    ...
    h->view_root_local = -1;   /* BUG-989: 0 is a valid local */
    return h;
}
 ...
 *
 * BUG-985: a GLOBAL-rooted entry is born `escaped` HERE, in the constructor.
...
typedef struct {
    ...
    int view_count;
    bool view_overflow;
    int view_root_local;  /* BUG-989: a pointer copy views what its source views */
...
static void ir_apply_alias(IRHandleInfo *dst, const IRAliasSnapshot *snap) {
    ...
    dst->view_root_local = snap->view_root_local;   /* BUG-989 */
}
...
/* BUG-1034: alloc_id 0 is the "no allocation identity / untracked" SENTINEL —
...
/* BUG-988: does reading a value of this type yield a REFERENCE into an
...
/* BUG-989: is this local a whole VALUE aggregate (struct / union / array) —
 ...
 *
 * BUG-984: EVERY entry rooted at the local escapes with it — the bare handle
 ...
 * enforces the escaped invariant at construction — BUG-985.) */
...
static IRPathState ir_merge_states(IRPathState *states, int state_count) {
    ...
    for (int si = first_live + 1; si < state_count; si++) {
        ...
        for (int hi = 0; hi < result.handle_count; hi++) {
            ...
            /* BUG-988: freed-then-reset is DEFINITE only when every pred says
            ...
            /* BUG-989: a view root survives the join only when every pred that
            ...
            /* BUG-1035: UNION the view sets. `view_alloc_ids` records the
...
/* BUG-985: the ONE key for a GLOBAL-rooted projection (`g.p`, `g_arr[0].q`).
 *
...
static int ir_extract_compound_key(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    *out_path_len = 0;
    ...
    /* BUG-989: a PROJECTION through a pointer VIEW of a whole local aggregate
    ...
    if (local < 0) {
        /* BUG-985: a GLOBAL-rooted projection resolves to the same
...
/* BUG-1035: fill `dh`'s VIEW SET from a multi-view call result.
 *
...
static bool ir_register_alloc_result(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    if (mc == IRMC_ALLOC || mc == IRMC_ALLOC_PTR) {
        ...
        if (h) {
            ...
            h->state = IR_HS_ALIVE;
            h->alloc_line = line;
            h->alloc_id = ir_alloc_id_of_local(dest);   /* BUG-1034: never 0 */
...
/* BUG-984: the FIELD / INDEX target of the assign spelling —
 *
...
/* BUG-985: slot-to-slot copy — `b.p = a.p`, `g.q = h.arr[0]`. The value is a
...
/* BUG-985: may the slot-clear rule treat this stored value as UNTRACKED?
 *
 ...
 *   a call                  -> the alloc-registration arm (BUG-984) or a summary
 ...
 * for a GLOBAL projection keyed under IR_GLOBAL_ROOT_ID (BUG-985) — the full
...
/* BUG-985: is `target` EXACTLY the slot of a tracked compound entry? A plain
...
static bool ir_assign_target_is_tracked_slot(ZerCheck *zc, IRFunc *func,
    ...
    if (target->kind == NODE_IDENT) {
        /* BUG-988: the BARE sibling — `p = null;` after `free(q)` where `q`
...
static void ir_check_expr_uaf(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    case NODE_ASSIGN:
         ...
         * BUG-985: a plain `=` whose target IS a tracked slot overwrites the
...
static bool ir_register_global_field_store(ZerCheck *zc, IRPathState *ps,
                                           ...
                                           int rhs_local) {
    /* BUG-985: the key is the ONE shared query — the same string
    ...
    if (grh && grh->state == IR_HS_ALIVE && grh->alloc_id != 0) {
        ...
        IRHandleInfo *gh = ir_find_compound_handle(ps, IR_GLOBAL_ROOT_ID,
                                                   gpath, total);
        if (gh) {
            if (gh->state == IR_HS_FREED) gh->freed_then_reset = true;   /* BUG-988 */
...
/* BUG-986: replicate every COMPOUND entry rooted at `src_root` onto
...
/* BUG-986: `slot = struct_value` — the target is a FIELD / INDEX slot and the
...
static void ir_check_inst(ZerCheck *zc, IRPathState *ps, IRInst *inst, IRFunc *func) {
    ...
    case IR_COPY: {
         ...
         * BUG-986: the replicate is now `ir_carry_compounds`, shared with the
    ...
    case IR_FIELD_READ: {
        ...
        if (inst->expr && inst->expr->kind == NODE_FIELD && inst->dest_local >= 0) {
            /* BUG-988: the shared predicate — `?*T` fields now alias too. */
                ...
                if (ir_extract_compound_key(zc, func, ps, inst->expr->field.object,
                    ...
                    if (fh && fh->alloc_id != 0) {
                        ...
                        if (ir_extract_compound_key(zc, func, ps, inst->expr,
                            ...
                            /* BUG-988: a PARAM's field is a LIVE value the caller
    ...
    case IR_ASSIGN: {
        ...
        if (inst->expr && inst->expr->kind == NODE_ASSIGN &&
            ...
            if (rv && rv->kind == NODE_CALL && rv->call.callee &&
                ...
                if (cs && cs->returns_param_color > 0) {
                    ...
                    /* BUG-1035: the MULTI-VIEW sibling of the arm above. It was
        ...
        if (inst->expr && inst->expr->kind == NODE_ASSIGN) {
            ...
            /* BUG-986: `h.inner = i` — a struct VALUE into a slot carries its
            ...
            if (target_expr && rhs_local < 0 && store_src_local < 0 &&
                ...
                /* BUG-985: an UNTRACKED store into a LOCAL-rooted tracked slot
                ...
                if (ir_extract_compound_key(zc, func, ps, target_expr,
                    ...
                    if (ch) {
                        /* BUG-988: keep the FACT that it was freed for the summary */
            ...
            ir_register_global_field_store(zc, ps, func, target_expr,
                                           store_src_local);
            /* BUG-985: slot-to-slot copy (`b.p = a.p`), AFTER G5 so a global
            ...
            /* BUG-989: the ASSIGN spelling of a view — `hp = &h;` (re)binds the
            ...
            if (acall && acall->kind == NODE_CALL) {
                ...
                if (amc == IRMC_ALLOC || amc == IRMC_ALLOC_PTR ||
                    ...
                    if (atgt >= 0) {
                        ...
                        /* BUG-984: FIELD / INDEX target (local or global root). */
        ...
        if (inst->dest_local >= 0 && inst->expr) {
                ...
                if (target && !used_compound) {
                    ...
                    if (base_local >= 0) {
                        ...
                        if (base_h && base_h->alloc_id != 0) {
                            ...
                            /* BUG-989: `%t = &h` where h is a whole local
            ...
            if (rhs && (rhs->kind == NODE_FIELD || rhs->kind == NODE_INDEX)) {
                ...
                if (ir_extract_compound_key(zc, func, ps, rhs, &fsrc_root,
                    ...
                    /* BUG-988: a PARAM's field read through THIS lowering
            ...
            if (inst->expr->kind == NODE_IDENT) {
                ...
                if (src_local >= 0) {
                    ...
                    if (is_move) {
                         ...
                         * ir_apply_alias. */
                        /* BUG-988: a PARAM read into a local — the orelse
    ...
    case IR_RETURN: {
        ...
        if (!rexpr && inst->src1_local >= 0 &&
            ...
            if (ir_should_track_move(ret_type)) {
                ...
                /* BUG-984: the compounds a returned by-value aggregate
        ...
        if (primary && primary->kind == NODE_IDENT) {
            ...
            if (ret_local >= 0 && ret_local < func->local_count) {
                ...
                if (ir_should_track_move(ret_type)) {
                    ...
                    /* BUG-984: the bare entry AND the compounds a returned
    ...
    case IR_FIELD_WRITE: {
        ...
        /* BUG-986: a struct VALUE written into the field carries its compounds. */
        ir_store_struct_value_into_slot(zc, func, ps, target_expr, rhs_local);
    ...
    case IR_STRUCT_INIT_DECOMP: {
        Node *si = inst->expr;
        /* BUG-1020: J3 (above) registered the ALIAS but left this case the only
        ...
        if (si && si->kind == NODE_STRUCT_INIT && inst->dest_local >= 0 &&
            ...
            for (int i = 0; i < inst->call_arg_local_count; i++) {
                ...
                if (vloc < 0) continue;
                /* BUG-1020 (move half): a move struct handed to a designated
                ...
                if (!vh || vh->state != IR_HS_ALIVE || vh->alloc_id == 0) {
                    /* BUG-986: no bare handle — the field value is a STRUCT
...
bool zercheck_ir(ZerCheck *zc, IRFunc *func) {
        ...
        if (pc > 0) {
            ...
            for (int bi = 0; bi < func->block_count; bi++) {
                ...
                for (int i = 0; i < pc; i++) {
                    ...
                    /* BUG-988: this return is the NULL PATH of param i's OWN
                     ...
                     * frees some field; maybe = some path does. */
                    {
                        ...
                        for (int hh = 0; hh < ps->handle_count; hh++) {
                            ...
                            /* BUG-988: a slot freed and then RESET (`h.p = null;`)
                    ...
                    /* BUG-988: look through `?` — an OPTIONAL pointer param
    ...
    for (int bi = 0; bi < func->block_count; bi++) {
        ...
        for (int hi = 0; hi < ps->handle_count; hi++) {
            ...
            /* BUG-1036 (latent): the four report sites below index
