...
static void emit_rewritten_node(Emitter *e, Node *node, IRFunc *func) {
    ...
    case NODE_ASSIGN: {
         ...
         * BUG-604 fixed binary `<<`/`>>` in this function; the compound-assign
         * versions were missed. Without _zer_shl/_zer_shr, raw C `<<=`/`>>=`
         * with n >= width is undefined behavior. Mirror emit_expr line 1375+:
         * emit `target = _zer_shl(target, n)`; hoist target via pointer if
         * side-effectful. (IR lowering already extracts call/orelse out of
         * targets, so the simple form is normally safe — pointer-hoist is
         * defense-in-depth for any nested NODE_CALL/NODE_ASSIGN that survived.) */
        if (node->assign.op == TOK_LSHIFTEQ || node->assign.op == TOK_RSHIFTEQ) {
            bool shift_side_effect = false;
            {
                Node *n = node->assign.target;
         ...
         * BUG-604 covered binary `/`/`%`; compound versions were missed.
         ...
         * Hoist value to a temp via __typeof__ (avoids double evaluation and
         * resolves the type). For signed targets, also hoist target value to
         * compare against INT_MIN. */
        if (node->assign.op == TOK_SLASHEQ || node->assign.op == TOK_PERCENTEQ) {
