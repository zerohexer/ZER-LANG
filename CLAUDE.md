...
| Signed div overflow (INT_MIN/-1) | 1068 | `IR_BINOP` TOK_SLASH/TOK_PERCENT (BUG-604) |
| Signed compound div/mod (INT_MIN/-1) | 1361-1373 | `emit_rewritten_node` NODE_ASSIGN TOK_SLASHEQ/TOK_PERCENTEQ (BUG-610) |
| Division by zero (binary) | 1055 | checker forces compile-time guard (no IR work) |
| Division by zero (compound) | 1361-1372 | `emit_rewritten_node` NODE_ASSIGN TOK_SLASHEQ/TOK_PERCENTEQ (BUG-610, defense in depth) |
| Shift safety (binary `<<`/`>>`) | 1078 | `IR_BINOP` TOK_LSHIFT/TOK_RSHIFT (BUG-604) |
| Compound shift (`<<=`/`>>=`) | 1375-1407 | `emit_rewritten_node` NODE_ASSIGN TOK_LSHIFTEQ/TOK_RSHIFTEQ (BUG-610) |
...
**Lesson (BUG-610):** When auditing safety wrappers between AST and IR
...
vs NODE_ASSIGN handlers) and one fix often misses the other.
