...
| **The address of a PACKED field ("is this pointer naturally aligned?")** | `@atomic_*` (BUG-493), deref (BUG-786), **call argument (BUG-813)** | the shared predicate `addr_of_is_packed_field` + the `Symbol.is_packed_derived` annotation, and `tests/zer/packed_field_direct_ok.zer` as the over-rejection boundary. The emitter DROPS the packed-ness (`*u32 q = &p.b` emits a plain `uint32_t*`), so nothing downstream can recover the fact — every sink must refuse the address itself. The call sink is the one that needed a boundary rejection rather than a propagated fact: the callee's `*u32` signature is the lie |
