...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **Architecture after Phase 9:** Simple expressions (ident, literal, binary, unary, field, index, typecast, struct_init, simple call, field/index callee call) → decomposed to IR ops with local IDs. Complex expressions (builtins, intrinsics, orelse, complex assignments) → IR_ASSIGN passthrough to emit_expr. The passthrough shrinks as each expression type is decomposed.
