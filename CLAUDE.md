...
| **An intrinsic's ADVERTISED runtime check** ("does the trap the docs promise actually get emitted for THESE operand types?") | `@pun` type_id (BUG-985), `@inttoptr` enum reads (BUG-986), `@container` provenance (BUG-984) | NO auto-gate — a checklist question. For each intrinsic that promises a runtime check, ask **for which operand types is the emitted check a compile-time constant, or absent entirely?** `@pun`'s was `type_id != TGT && type_id != 0` where only struct/enum/union carry an id, so any primitive operand folded it to false. The tell is a comment that already NAMES the mechanism as broken while fixing only one consequence of it |
