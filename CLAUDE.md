...
| VRP range JOIN / POSITION (where an expression is evaluated vs where it is checked) | NODE_IF (§C#13 ✅), switch-arm, for-body, while/do-while body, do-while first-iter, goto/label — **the loop CONDITION** (BUG-1015: evaluated per iteration under the loop-carried range; all three drivers checked it under the pre-loop range) — **and the for STEP** (BUG-1017: `for (k=0; k<8; k += a[k])` on a `u32[4]` proved `a[k]` against k in [0,0], emitted a bare `k += a[k];`, ASan stack-buffer-overflow, zero diagnostics). Only FIXED ARRAYS miscompile — a slice keeps its dynamic `.len` check, which is why no slice test could ever see either bug | **`tests/test_vrp_position_matrix.c`** (11th matrix, in `make check`) — the checklist leaked TWICE so it is a gate now. 17 cells, construct x position: STALE cells must produce a bounds VERDICT (warning or error), PROVEN cells must produce SILENCE. Works because a proven fixed-array index emits literally nothing. Verified to FIRE: 4 silent holes against the pre-BUG-1017 build. **A new control-flow kind, or a new POSITION an existing one evaluates, needs its cell in the SAME commit** |
