...
/* BUG-974: one entry of the read-modify-write value taint — "this LOCAL's value
...
typedef struct {
     ...
     * construction — hooks the NODE_RETURN check, so it sees every valued return
    ...
    /* BUG-998: while true, collect_shared_types_in_expr skips the statement's
     * DIRECT shared accesses and collects ONLY the callee-transitive types (the
    ...
    int branch_depth;
    /* BUG-958 — loop-induction CERTAINTY. A VRP range says which values a
    ...
    /* BUG-974: cross-statement read-modify-write taint for the ISR / spawn
     * sharing rule.
     *
    ...
    /* BUG-960: the two halves of `after_spawn_in_func`, kept apart so a join can
    ...
    struct IsrGlobal {
        ...
        /* BUG-948: a STATIC LOCAL, identified by its declaration node (two
...
/* BUG-995: mirror node-keyed marks onto a cloned node (ast_clone hook). */
