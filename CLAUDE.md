...
**ASan MAY BE A NO-OP IN YOUR SANDBOX — verify it before trusting a clean run (2026-09-02).**
In the cloud/container environment this project is sometimes audited from, a hand-written
...
establish memory findings the way BUG-978 was: read the emitted C for the guard, and make the
...
| **Cast policy ("what does `(T)x` emit?")** | `emit_expr` NODE_TYPECAST (global inits), `emit_rewritten_node` NODE_TYPECAST (plain assigns, **defer bodies**, spawn args), `emit_ir_inst` IR_CAST (3AC) | **CLASS-KILLED 2026-09-02 (BUG-979): `classify_cast()` + `emit_cast_value()` hold the decision AND the emission once; each site supplies only its operand via `CastOperand`.** The form enum has no `default:`, so a new cast form is a build error at all three. `grep -c "type mismatch in cast" emitter.c` must stay **1** — a 2 means someone re-inlined the policy. Pre-kill, sites 1+3 had 4 behaviours and site 2 had 1, so `(bool)5` stored 5 (truthy AND != true) and a `*opaque` cast lost its type-id trap |
| **Funcptr-return declarator ("does this function RETURN a funcptr?")** | IR signature emitter, AST prototype emitter | ONE query **`funcptr_return_shape()`** (peels `distinct` + null-sentinel `?`). BUG-980 was BUG-879 one sink over: the array-element site peeled both wrappers, the return site tested `->kind == TYPE_FUNC_PTR` raw, so `?VFn f()` emitted an abstract declarator and GCC refused a valid ZER program |
| **Shared-struct AUTO-LOCK ("was the mutex actually EMITTED?")** | every statement-body form a shared access can sit in (function body, if/else, for, while, do-while, switch arm **braced and BARE**, `@critical`, `@once`, defer braced and bare, nested block) | **`tests/test_sharedlock_matrix.c`** (NEW 2026-09-02, BUG-981). Reads the EMITTED C, because the defect produces no diagnostic AND no single-threaded runtime signal — a data race is not observable from one thread. The accept/reject matrices structurally cannot see this class: both spellings are accepted. A bare switch arm (`0 => g.x = 5,`) emitted an unsynchronized write to shared memory |
| **Enum-forge doors** ("can this conversion produce a non-variant?") | `@bitcast`, `@truncate`, `@saturate` — and `@cast` verified NOT to be one | the three `tests/zer_trap/*_enum_forged_*.zer` PLUS the consumer-side guard: an exhaustive enum `switch` re-checks the elided last arm and traps (BUG-930, `enum_nonvariant_trap` at both emitter paths), which covers the doors no conversion opens — auto-zero, an `@inttoptr`/`@pun` load, a struct field. Patched THREE times across three sessions before the door set was written down; each fix closed one door and left the siblings |
| **Documented examples** ("does the reference still compile — and does it still get REJECTED where it says it does?") | every ```zer block in `docs/reference.md`, in BOTH directions | `tools/audit_reference_examples.sh` + `tools/reference_example_baseline.txt` — IN `make check` (the 8th gate). Twice the docs asserted a feature the parser did not have (funcptr arrays, bit-extract through a pointer). A block illustrating a REJECTION is skipped by default; give it `<!-- audit: expect-error: <substring> -->` to assert the rejection AND its reason (BUG-919 — two `// COMPILE ERROR` container examples compiled clean). Never assert only "must fail": most such blocks are fragments that die on syntax or `undefined identifier` first, so the bare form passes vacuously — same weak-oracle shape `// expect-error:` closes for `tests/zer_fail/` |
| **Rendering a `double` into emitted C** | every float-literal emission site (was FIVE) | ONE helper `emit_double_lit` + **`tools/audit_float_literal.sh`** (BUG-983). `%.17g` is exact for FINITE doubles and prints the non-C tokens `inf`/`-inf`/`nan` for the other three, so `f64 x = 1e400;` emitted `x = inf;` and GCC blamed the user's `.zer` line. The gate fails the build on any float conversion specifier outside the helper — verified RED by reverting one site |
| **An intrinsic's ADVERTISED runtime check** ("does the trap the docs promise actually get emitted for THESE operand types?") | `@pun` type_id (BUG-985), the `*opaque` ERASURE that feeds it (BUG-987), `@inttoptr` enum reads (BUG-917), `@container` provenance (BUG-915) | NO auto-gate — a checklist question. For each intrinsic that promises a runtime check, ask **for which operand types is the emitted check a compile-time constant, or absent entirely?** `@pun`'s was `type_id != TGT && type_id != 0` where only struct/enum/union carried an id, so any primitive operand folded it to false. Two follow-on habits, each of which paid: **run the same probe against the sibling intrinsic** (`@pun` and `@ptrcast` share `_zer_opaque`, and the sibling held the ROOT cause), and **check what the check READS, not just what it tests** — the erasure was recording `type_id = 0`, a sentinel meaning "unknown origin, FFI floor", for pointers whose type the compiler knew exactly. The tell is a comment that already NAMES the mechanism as broken while fixing only one consequence of it |
| **A sentinel with two meanings** | `type_id == 0` = "no id for this kind" AND "unknown origin, do not check" (BUG-987) | NO auto-gate. When a value has a reserved sentinel, write down which question it answers. Twelve producer sites meant the first; the single consumer read the second, and the gap was a silent OOB |
...
**When the defect has NO runtime signal at all, assert on the EMITTED C** — that is the third
observable. `tests/test_sharedlock_matrix.c` (BUG-981) greps the generated file for the mutex on
the specific global it wrote, because a missing lock is a data race and a data race cannot be
observed from a single-threaded test at all. Two rules make it non-vacuous: match something
SPECIFIC (`pthread_mutex_lock(&g._zer_mtx)`, not a bare `mutex` — the emitted preamble defines
helpers that mention it), and assert the operation under test was emitted AT ALL, so a cell whose
program silently stopped containing the shared write reports VACUOUS rather than passing.

**WHAT A GREEN `make check` ACTUALLY COVERS (verified from a run, 2026-09-02 — do not quote from
...
holds if the decision is WRITTEN DOWN. BUG-978 was a live silent out-of-bounds stack write whose
...
no sentence explaining why the skip was correct. **The gate found the bug and the baseline buried
it.** RULE: every row (or contiguous block of rows) you add needs the sentence that says why the
omission is right. A block header naming the function is NOT a justification. When you review this
file, treat any run of rows with no prose above it as unaudited, not as settled.
...
walker written as `if (k == NODE_X) ... else if (k == NODE_Y) ...` is covered by neither, and
several in `checker.c` say so in their own comments ("if/else chain ... to keep the
walker_default_audit discipline" — which reads as discipline and functions as an exemption).
BUG-977 and BUG-978 were both if-chains whose trailing comment asserted "all other kinds are
leaves"; both assertions were false, for `NODE_SPAWN` (`spawn_stmt.args[]`), `NODE_AWAIT`
(`await_stmt.cond`) and `NODE_ASM` (operand exprs). When you touch a safety if-chain, convert it to
a no-`default:` switch in the same commit and baseline its intentional no-ops with reasons. Cost of
converting: ~30 lines of `case` labels. Cost of not: two holes that survived four sessions of work
on the very axis they belong to.
...
| Enum forging (a value outside the variant set) | Runtime variant guard at the point of forgery — TRACKED, not banned, so reading an enum from a register still works (as `@bitcast(State, *u32reg)`). The GUARDED doors are EXACTLY THREE: `@bitcast` (BUG-843), `@truncate` (BUG-891), `@saturate` (BUG-910). `@cast` is NOT a door — it requires a distinct typedef and cannot name a bare enum. `@inttoptr` to an enum-carrying pointee WAS a fourth route (BUG-986: `volatile *State reg = @inttoptr(*State, a); switch (*reg)` returned an arm on a register holding 200, zero guards emitted) and is now **REJECTED**, keeping the guarded set at three — guarding it would have meant firing at every READ through the pointer (deref/field/index/nested), a new N-sink surface, and the rejection routes users to the guarded `@bitcast` idiom the docs already teach. The guard recurses struct fields, optional payloads and array elements, and each door wires it at BOTH emitter dispatch paths. **Adding a new value-producing conversion? It is a fourth door — wire the guard. Adding a new way to POINT AT foreign memory? Ask whether an enum can be read through it.** The reason every one of these reports as severe: the exhaustive-switch lowering emits the LAST arm as an unconditional `else`, so a non-variant value *takes an arm* instead of falling through — see docs/limitations.md "the last-arm elision is the amplifier". |
