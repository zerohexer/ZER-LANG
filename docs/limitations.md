...
- ~~**i64 literal range** (`lzmkhn`): `i64_literal_above_max`, `_below_min`,
  `_over_range_sinks`.~~ — **CLOSED 2026-09-12 as BUG-992** (all three reject; the
  boundary positive runs).
- ~~**compound float <-> int** (`fhf8rn`): `compound_float_into_int`,
  `compound_int_into_float`.~~ — **CLOSED 2026-09-12 as BUG-995** (both reject;
