...
## OPEN — Concurrency memory-safety: 3-sweep audit (~25 holes), four-axis closure design (2026-06-20)
...
verified data races + cross-thread use-after-free that compile clean today. Full
...
**Do NOT claim ZER is data-race-safe as shipped** — it is *designed* to be (auto-
inferred Rust-equivalent safety) but NOT yet complete. **Spec NOT frozen.**
...
checker.c:~8676 + spawn-arg handler ~12005). Every exclusion / forgotten type-kind
is a hole:
- `volatile` whitelisted (8686) → thread-shared `volatile` compound-RMW race; the
  ISR volatile-RMW guard (`check_interrupt_safety`, 13617) is `in_interrupt`-gated,
  no thread arm. Compiler even *recommends* `volatile` as a race fix (12127).
- `Arena` whitelisted (8698) → concurrent `arena.alloc()` races bump metadata.
- `threadlocal` whitelisted + `&threadlocal` untainted → publishing `&tl` to a
  shared/global carrier = cross-thread wrong-TLS/UAF.
- `TYPE_SLICE` + `TYPE_OPAQUE` spawn args uncased → `[*]T` over stack array, or
  `(*opaque)&local`, passes (local-derived walker also misses `NODE_TYPECAST`).
- Fix: exhaustive spawn-arg type-kind switch (`-Wswitch`-style gate); carrier-or-
  tainted *inclusion* model; generalize `check_interrupt_safety` to any-two-
  contexts; wire `zer_volatile_compound_valid` (concurrency_rules.c:106, never
  called).

**Axis B — single-root auto-lock incompleteness** (per-statement
`current_stmt_shared_root`, ir_lower.c). Locks only the first shared root; bypassed
at: `shared(rw)` multi-read (BUG-500 skip); union-switch arm (lock released, live
ptr into shared bytes); `@cond_wait` predicate (2nd shared read unlocked,
`collect_shared_types_in_expr` no `NODE_INTRINSIC` case); `@once` loser (no wait →
half-constructed read); **defer-body shared access** (`find_shared_root_in_stmt_ir`
NULL for `NODE_DEFER`, ir_lower.c:~1228). Fix: one lock-scope-walker redesign
covering all roots + switch-arm/defer/cond bodies + `@once` loser-wait.

**Axis C — per-function CFG lattice (lifetime/temporal).** **MOST ACTIONABLE BUG
(verified at code level):** `ir_merge_states` (zercheck_ir.c:573-643) unions ONLY
`handles[]`; `threads[]`/`joined` ride `ir_ps_copy(first_live)` and are NEVER merged
from other predecessors; convergence check (~4228) ignores thread state. So a
scoped-spawn `ThreadHandle` created in one branch (or label-reordered pred) is
**dropped at the CFG merge**, the return-block join scan (~4970) sees empty
`threads[]`, emits no "not joined" → because `&stack-local` into a scoped spawn is
allowed ONLY on the join premise (checker.c:12011), this is a **false-green
cross-thread stack UAF — the enforcer exists but is bypassed at every merge.** Also:
stack-local `shared struct` to fire-and-forget spawn (12009 accepts `*shared T`, no
lifetime check); free-after-publish; detached grandchild outliving a scoped join;
block-scoped Barrier/Semaphore outlived by function-scoped join; move-struct
`&`-unwrap missing in `IR_SPAWN` key extraction (sibling `IR_CALL`/asm sites unwrap
`&`); async task struct raced. Fix: extend `ir_merge_states` + convergence
comparator + `ir_ps_copy` to union/compare `threads[]`; add block-scope carrier
lifetime tracking; publication lifetime-arm + Handle-gen runtime trap for freeable
carrier payloads. (Tripwire to write: a scoped `spawn worker(&local)` inside an
`if` branch with the `.join()` in the same branch must still be tracked across the
merge.)

**Axis D — boundary/runtime concurrency-capture (NEW, sweep 3 — why spec is NOT
frozen).** Concurrency entering with no visible ZER node, so A/B/C cannot attach:
(1) FFI/cinclude — ZER ptr/funcptr handed to a bodyless extern that
`pthread_create`s internally (`scan_unsafe_global_access` dead-ends at missing body
8757; `keep` is lifetime-only, never thread-safety; `IRThreadTrack` keys off ZER
spawn only). (2) Compiler-emitted runtime — `@probe`'s `static` (not `__thread`)
`_zer_in_probe`/`_zer_probe_jmp` + process-global signal handler (emitter.c:~5056)
raced by threads; a ZER fn as a POSIX signal handler self-deadlocks its non-
recursive mutex. Fix is a boundary DISCIPLINE: a `threads`/`captures` capability on
cinclude params (+ `--strict-interop` default-reject for non-`shared`/non-static ZER
pointers to thread-capturing externs); make emitted runtime `__thread`/spawn-guarded.
Partly "the existing C-interop floor, applied to concurrency — make it audit-visible".
...
**Status:** design converged on 4 axes (each with a known general shape + single
structural fix), but **spec NOT frozen** (Axis D appeared in sweep 3) and
implementation NOT started. **Subsystem-scale** (like `keep` / the IR migration),
not patches. Still-unprobed residue to settle before a "closure" claim: full FFI
callback-table surface; other emitter-runtime statics (`@once` state, async task,
shared-lazy-init CAS) for `__thread` vs `static`; a systematic "merged vs
first_live-only" audit of EVERY `IRPathState` field (the Axis-C bug class is "a
second lattice family not added to the merge"); cross-module spawn/extern; and
`NODE_STRUCT_INIT` global read inside a spawn body (checker.c:8807 leaf list returns
false). When starting implementation, **fix the Axis-C `ir_merge_states` thread
merge first** — it is a concrete soundness bug in a shipped enforcer (false-green
UAF), highest severity, clearest fix, and re-verify it personally before editing.
