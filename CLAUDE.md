...
|---|---|---|
| Escape / keep ("frame-bound?") — **and its LAUNDER-PEEL list** (`unwrap_ptr_launder`): a value shape peeled at one sink and not another is the same hole. `NODE_TYPECAST` was missing from the shared peeler for the entire life of C-style casts (BUG-785) | store-global, return, keep-call, keep-infer, struct-field-store, spawn-arg | `tools/sink_matrix.sh` — **ADD A CELL per new shape** (a shape with no cell is INVISIBLE; yd5ajq grew it 32→41; the 2026-08-12 `p15` launder shape grew it 54→66 and its 6th cell found a live hole the report had not named). `make check-sink-matrix` |
...
| **Shared-struct lock root ("which shared struct does this expression touch?")** | primary lock emitter, multi-root lock, spawn-arg lock, defer-body lock, same-statement deadlock rule | **ONE query** `checker_walk_shared_chains` (checker.c, exported in checker.h) — a no-`default:` switch over NodeKind, so a new kind FAILS the build. All five sites delegate to it. Fix by extending the ONE walk, never by adding a sixth copy; see compiler-internals.md "The unified shared-struct chain walk" |
...
| Non-volatile MMIO pointer | `@inttoptr` at a CONSTANT address inside a declared `mmio` range yields a `volatile *T`, so binding it to a plain `*T` is a compile error (BUG-788). Without this GCC deletes repeated peripheral stores and constant-folds status reads. A VARIABLE address is not forced volatile (the `@ptrtoint`+math+`@inttoptr` round-trip must still produce a plain `*T`) — residual tracked in limitations.md |
