...
137. **Volatile array → slice coercion rejected (BUG-310)** — TYPE_SLICE has no `is_volatile` flag. Passing a volatile array as a slice strips volatile — GCC optimizes away MMIO reads. Checker rejects at 3 sites: call args (after BUG-263 volatile pointer check), var-decl init (after const slice check), NODE_ASSIGN (inside BUG-245 const array block, checks `vsym->is_volatile`). Non-volatile arrays still coerce to slices normally.
...
112. **RF11: `expr_is_volatile()` / `expr_root_symbol()` helpers** — Walks any expression through NODE_FIELD/NODE_INDEX/NODE_UNARY(STAR) to root NODE_IDENT, looks up symbol in current then global scope. Returns symbol or NULL. `expr_is_volatile()` wraps this to check `sym->is_volatile`. Used by array assign (BUG-273), if-unwrap (BUG-272), switch capture (BUG-274), @cstr (BUG-223). New volatile emission sites should use this instead of inline walks.
