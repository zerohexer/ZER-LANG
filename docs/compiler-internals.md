...
- **Remaining (Phase 8b completion):** 20 `emit_expr` calls in `emit_ir_inst`. IR_ASSIGN/IR_CALL/IR_BRANCH/IR_RETURN/builtins/IR_INTRINSIC still use Node *expr. Need: (1) expand `try_decompose` to more expression types (NODE_FIELD on locals, NODE_INDEX, NODE_CALL). (2) Replace emit_expr in IR_BINOP/IR_UNOP/IR_FIELD_READ/IR_INDEX_READ handlers with local-ID emission. (3) Verification: `grep emit_expr` in emit_ir_inst body = 0 (except IR_NOP passthrough).
