    ...
    if (!is_temp && !is_param) {
        for (int i = 0; i < func->local_count; i++) {
            if (func->locals[i].orig_name_len == name_len &&
                ...
                /* BUG-961: a CAPTURE may only dedup with another CAPTURE, and a
...
int ir_clone_blocks_from(IRFunc *func, Arena *arena, const IRBlock *src, int n, int src_first) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        memcpy(dst->insts, sb->insts, (size_t)sb->inst_count * sizeof(IRInst));
        dst->inst_count = sb->inst_count;
        dst->inst_capacity = sb->inst_count;
...
bool ir_validate(IRFunc *func) {
     ...
     * unreleased lock at runtime. Logged as ERROR, aborts compilation. */
    {
        ...
        /* Guard the count: GCC's range analysis cannot prove block_count is
         * non-negative here, and a negative int converts to a huge size_t —
         * calloc would then be asked for an absurd allocation (or, worse,
         * succeed on an overcommitting kernel). */
        bool *fire_in_block = (func->block_count > 0)
            ? (bool *)calloc((size_t)func->block_count, sizeof(bool)) : NULL;
        if (fire_in_block) {
            for (int bi = 0; bi < func->block_count; bi++) {
                IRBlock *block = &func->blocks[bi];
                for (int ii = 0; ii < block->inst_count; ii++) {
