...
/* find_shared_root_in_stmt() was DELETED 2026-08-15. Per-statement shared-struct
 * lock placement is decided in ir_lower.c (current_stmt_shared_root) and emitted
 * from the IR — the emitter's AST-side copy of that question had no callers left
 * after the IR migration. find_shared_root(), which it wraps, is still live. */
