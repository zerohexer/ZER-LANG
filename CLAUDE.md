...
// spawn — thread creation with safety enforcement:
...
| Thread data race | `shared struct` auto-locked. Non-shared pointer to `spawn` → compile error. Handle to `spawn` → compile error. |
