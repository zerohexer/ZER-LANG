...
| Alias/ownership laundered through a deref | Binding a POINTER, `Handle(T)` or move struct read out of `*p` -> compile error (the compiler cannot prove which allocation it refers to) |
| Mixed atomic / plain access | A location touched by `@atomic_*` anywhere is an ATOMIC CELL: a plain read or write of it in a concurrent context -> compile error. Keyed on the whole path (`g.i.n`, `arr[0]`, `g.n[0]`) |
| Non-atomic RMW in an ISR | A `volatile` global compound-assigned from an interrupt AND touched by main -> compile error, including when the RMW happens inside a helper called with `&g` |

---

### The dereference identity rule

Reading a value out of a pointer dereference makes a COPY. For a plain scalar or struct that is
exactly what you want. For three kinds of value it destroys the identity the compiler tracks, so
binding the result is a compile error:

```zer
*Node k = *pp;          // ERROR - pointer ALIAS: which allocation does k refer to?
Handle(T) k = *hp;      // ERROR - Handle: which pool slot does k own?
Tok b = *tp;            // ERROR - move struct: b would be a second owner
pool.free(*hp);         // ERROR - the callee's free cannot be attributed to any allocation
consume(*tp);           // ERROR - same, for a move struct

u32 v = *p;             // OK - a scalar copy has no identity to lose
S copy = *sp;           // OK - a plain struct VALUE copy
use_ptr(*ppp);          // OK - a plain pointer argument is a borrow for the call
```

Recovering the identity would need points-to analysis over the dereferenced pointer, which ZER
deliberately does not have (whole-program analysis is outside the architecture). The rule is
therefore "cannot prove => reject". **The fix is always to bind the pointer directly:**

```zer
*Node k = p;            // alias the pointer the compiler CAN follow
...
zerc source.zer --stack-limit 2048                # error if a frame or call chain exceeds N bytes
zerc source.zer --emit-ir                         # print the IR and exit (debugging)
zerc source.zer --track-cptrs                     # force *opaque runtime tracking when emitting C
zerc source.zer --trace                           # print the compilation flow to stderr
```

An unrecognised `--option`, and an unrecognised `--target-arch=` value, are HARD ERRORS — a
mistyped safety flag must never leave the check silently disabled. `--release` is accepted for
compatibility but has no effect and says so: pointer-tracking safety is always compiled in.
