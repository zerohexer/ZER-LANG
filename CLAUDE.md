...
| IR Phase 1-8 (data structures, lowering, 3AC, emission, zercheck, VRP) | Done | Done. Migration: --use-ir, **195/195 ZER + 761/761 rust, 0 fail, 0 hang**. Phase 8: (a) on-demand locals, ident rewriting, scope conflict BUG-507, async yield BUG-508/509/510. (b) `lower_expr()` wired for ident/literal/binary/unary/field/index/string → IR_COPY + 3AC ops emit from local IDs. (c) `can_lower_expr()` + conditions/returns decomposed. `emit_ir_value()` handles ident/lit/binary/unary/field DIRECTLY — only delegates to `emit_expr` for calls/builtins/index/intrinsics/orelse/casts/assignments/struct-init (1 fallback). 17 `emit_ir_value` call sites in `emit_ir_inst`. BUG-511: ir_find_local C-name match. BUG-512: always rewrite_idents before lower_expr (captures share condition expr). |
