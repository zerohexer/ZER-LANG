...
static void lower_orelse_to_dest(LowerCtx *ctx, int dest_local, Node *orelse_node, int line) {
    ...
    if (orelse_node->orelse.fallback_is_return ||
        ...
        ctx->func->blocks[bb_fail].orelse_fallback_local = tmp_id;   /* BUG-1004 */
    }
    if (orelse_node->orelse.fallback_is_return) {
        ...
        if (ctx->current_block != bb_fail) {
            ...
            ctx->func->blocks[ctx->current_block].orelse_fallback_local = tmp_id;   /* BUG-1004 */
        }
...
/* BUG-1003: THE per-statement wrapper — bounds guards, then the shared-struct
...
/* BUG-1003: a BODY position that the grammar may fill with either a block or a
...
static void lower_stmt(LowerCtx *ctx, Node *node) {
    ...
    case NODE_BLOCK: {
        ...
        /* BUG-1003: the per-statement guard/lock/unlock wrapper is ONE function
    ...
    case NODE_SWITCH: {
        ...
        for (int oi = 0; oi < node->switch_stmt.arm_count; oi++) {
            ...
            /* BUG-1003: an expression-form arm (`1 => g.v = 5,`) is a bare
    ...
    case NODE_DEFER: {
        ...
        if (ctx->defer_count < ctx->defer_bodies_cap) {
            ...
            if (!defers_stay_on_ast(ctx)) {
                ...
                /* BUG-1003: `defer stmt;` is a bare NODE_EXPR_STMT body. Route it
