...
# Systematic exhaustive shape×violation matrix (drives the zerc binary, like
# test_semantic_fuzz but deterministic + full-grid). -Wswitch enforces that
# the grid can't silently shrink.
test_shape_matrix: tests/test_shape_matrix.c
	$(CC) $(CFLAGS) -o $@ $^

# Exhaustive escape/lifetime (the `keep` axis) soundness guard. NEGATIVE-ONLY:
# every cell is an unsafe local-escape that MUST be rejected for the escape
# reason. -Wswitch enforces the {dest x launder x src} grid can't shrink.
...
# Data-race / spawn / deadlock soundness guard (first non-memory frontier
# oracle). NEG: spawn non-shared ptr/global (incl. transitive), deadlock
# same-statement, spawn-in-@critical, ThreadHandle not joined (incl. one-branch).
# POS: shared auto-lock, scoped spawn+join, value args, separate-stmt, threadlocal.
...
# async (yield/await) soundness guard (frontier Domain 3, final). NEG: yield/
# await in defer, yield/await in @critical, spawn in async. POS: yield, await,
# defer-without-suspend, local across yield, await-on-shared (safe — lock
# released between polls). EMIT-ONLY harness.
...
# defer x goto ARMING oracle (2026-08-02). Unlike every other matrix this one
# checks a RUNTIME VALUE (acquire/release balance), because the class it guards
# is a silent miscompile — a defer firing on a path whose registration never ran
# — not a missing diagnostic. It is also the acceptance test for the durable
# §F2 fix: when the per-defer armed flag lands, drop cell_currently_rejected()
# and all cells must assert values.
test_defer_goto_matrix: tests/test_defer_goto_matrix.c
	$(CC) $(CFLAGS) -o $@ $<

# VRP control-flow POSITION oracle (2026-09-15, BUG-1017). ZER's value-range
# analysis runs on the AST, so every construct must hand-manage the range
# environment at every POSITION it evaluates an expression — a class CLAUDE.md
# listed with "NO auto-gate, checklist it". The checklist leaked twice
# (BUG-1015 the condition, BUG-1017 the step), so this is the gate. Oracle: a
# proven fixed-array index emits NOTHING, so "compiled in silence" is the exact
# signature of both correct elision and the hole. Verified to FIRE: 4 silent
# holes against the pre-BUG-1017 build, 0 after.
test_vrp_position_matrix: tests/test_vrp_position_matrix.c
	$(CC) $(CFLAGS) -o $@ $<

# BUG-1090..1100: the VRP FACT-VALIDITY grid — the other half of the position
# matrix. Not "is the index checked under the range that holds where it is
# evaluated?" but "is the recorded range still a TRUE fact about this variable?"
# (identity / constant / invalidation / hoist / summary). RUNTIME oracle: each
# cell's access is out of bounds at run time and returns 42 right after it, so
# exit 42 is an unchecked access. Verified to FIRE: 28 silent holes against the
# pre-fix build, 0 after.
test_vrp_fact_matrix: tests/test_vrp_fact_matrix.c
	$(CC) $(CFLAGS) -o $@ $<
...
check: zerc test_lexer test_parser test_parser_edge test_checker test_checker_full test_extra test_gaps test_emit test_firmware test_firmware2 test_firmware3 test_production test_fuzz test_semantic_fuzz test_shape_matrix test_escape_matrix test_keep_matrix test_cflow_matrix test_conc_matrix test_view_alias_matrix test_hw_matrix test_async_matrix test_asm_matrix test_defer_goto_matrix test_vrp_position_matrix test_vrp_fact_matrix test_ir_validate
	...
	./test_vrp_position_matrix
	./test_vrp_fact_matrix
	./test_ir_validate
	...
	@echo ""
	@echo "=== reference.md examples must compile ==="
