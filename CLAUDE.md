...
| **Allocation SPELLING ("did this program allocate?")** | var-decl INITIALIZER (`?Handle h = p.alloc();`) vs plain ASSIGNMENT (`?Handle h; h = p.alloc();`) — the two lower to different IR | BUG-919: the assignment form registered NO allocation, so leak + double-free + UAF + wrong-pool were ALL inert on it, for `alloc()`, `alloc_ptr()` and universal `alloc()`. A CONDITIONAL allocation can only be written that way, so every branch shape was unchecked too. `tests/zer_fail/alloc_assign_form_*.zer` cross spelling x sink. **Any new tracked-value registration must be tested in BOTH spellings** |
| **CFG merge field coverage ("what is the join of two disagreeing values?")** | every field of `IRHandleInfo` / `IRPathState` in `ir_merge_states` | a field present in the state but absent from the merge silently keeps the FIRST LIVE PREDECESSOR's value. Produced the `threads[]` false-green (2026-06-21) and again `pool_name` (BUG-918). Merge as an AMBIGUITY marker, not by picking a winner and not by clearing to NULL (clearing disables the check = the same hole one step removed). ~11 fields still unmerged — enumerated in `docs/limitations.md` |
| **Depth/recursion caps in a SAFETY predicate** | every `depth > N` in checker.c / zercheck_ir.c | the cap's return value IS a safety decision. For a predicate that turns tracking ON, returning `false` at the cap is ACCEPT-UNSAFE (BUG-917: use-after-move accepted at 34 levels of struct nesting, caught at 33). A cap bounds recursion; it must not decide safety. Audit each one for its DIRECTION, not its existence |
