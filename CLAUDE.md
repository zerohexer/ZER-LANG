...
| **Single-statement BODY positions ("does this statement get the per-statement guard + lock?")** | the NODE_BLOCK loop, the statement-form `defer stmt;` body, the expression-form `1 => expr,` switch arm | ONE wrapper **`lower_stmt_in_block`** (ir_lower.c) + **`lower_body`** for any position that may hold a block OR a bare statement (BUG-972). Both bare forms wrote a `shared struct` with NO mutex while the brace form one keyword away locked. Gate: REQUIRED fingerprints in `tools/emit_audit.sh` (verified red pre-fix). A parser change that produces a bare statement in a new body position must route it through `lower_body` |
| **Call-descent DEPTH CAPS ("what does the walker answer past its cap?")** | spawn race scan (5 `_scan_global_depth` gates), ISR walker + its two funcname siblings, post-spawn atomic-cell walk, `func_rmw_param_mask` | **FAIL CLOSED** (BUG-973): the cap sets a flag / returns the conservative value and the entry point REFUSES the program — never `return false` for a body it did not walk. Three of four failed open (a global RMW 40 calls below a spawn compiled clean); the fourth cached the wrong answer up the chain. `grep -n "depth > [0-9]" checker.c zercheck_ir.c` enumerates every guard; the ones NOT yet converted are ledgered in limitations.md — a new walker's cap must fail closed from day one |
...
|---|---|
| Buffer overflow | Inline bounds check on every array/slice access; proven-safe indices skip check (range propagation, OR the index's own type/shape — a `u8` into 256, `x >> 30` / `x & 3` / unsigned `x % 4` into 4, BUG-974; `uN`/`iN` and signed deliberately excluded); unsafe indices get auto-guard (silent if-return inserted) |
...
| Thread data race | `shared struct` auto-locked. Non-shared pointer to `spawn` → compile error (unless scoped with ThreadHandle+join). Handle to `spawn` → compile error. Spawn target body scanned for non-shared global access: error (no sync) or warning (has @atomic/@barrier). Transitive through callees (32 levels; a deeper chain is REFUSED as unanalyzed, BUG-973 — never reported clean). Escape: volatile, shared, threadlocal, @atomic_*. |
...
1. **`IR_GLOBAL_ROOT_ID (-2)` pseudo-root global tracking** — globals that
   ...
   escaped on a global entry — **enforced in the constructor since BUG-976**
   (`ir_add_compound_handle` sets it), because `ir_extract_compound_key` now
   resolves a global-rooted PROJECTION (`g.p`) to `(-2, "g.p")` at EVERY sink
   (before, G5 wrote the entry at the store and no other sink could find it).
   Prefer this pseudo-root reuse over adding PathState fields for any future
   "track non-local entity" need.
