...
static bool type_carries_enum_e(Type *t, int depth) {
    if (!t) return false;
    /* BUG-1005: by-value nesting is acyclic, so the guard is a safety net; past it
...
/* Build "<path><suffix>" on the heap. BUG-1005: the old `char sub[256]` silently
...
static void emit_enum_variant_guard_path(Emitter *e, Type *t, const char *path,
    ...
    if (!t) return;
    /* BUG-1005: this walker STOPPED at depth 8 — so an enum forged 11 structs
    ...
    if (k == TYPE_ARRAY) {
        ...
        if (!type_carries_enum_e(in, 0)) return;
        /* BUG-1005: `size > 4096 -> return` was a silent skip — a `Color[5000]`
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            if (node->intrinsic.type_arg) {
                ...
                /* BUG-1005: an ARRAY source DECAYS under `__auto_type` (`__auto_type
                ...
                /* BUG-1005: an ARRAY target (`@bitcast(Color[50], raw)`) needs a C
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            if (node->intrinsic.type_arg) {
                ...
                int tmp = e->temp_count++;
                int tmp2 = e->temp_count++;
                /* BUG-1005 / BUG-1005: IR twin of the AST-path handler above — an
