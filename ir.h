...
typedef struct {
    ...
    bool is_volatile;        /* `volatile T x;` — #19 (VOL-1): scalar/aggregate
                              * locals carry no volatile in their Type (only slice/
                              * pointer do), so the emitter needs this to emit the
                              * qualifier. Without it a `volatile u32 d;` delay/timing
                              * loop was silently optimized away on bare metal. */
...
typedef enum {
    ...
    /* BUG-994 (2026-09-05): unconditional runtime trap — a TERMINATOR (no
...
typedef struct IRInst {
    ...
    /* BUG-995 (2026-09-05): IR_DEFER_PUSH / IR_DEFER_FIRE are MARKERS. The
    ...
    /* BUG-995: provenance of an instruction lowered from a defer BODY —
    ...
    int defer_origin;
    /* BUG-995: IR_BRANCH gate around an inlined defer body. 1 = the F2 ARMED
