...
## OPEN — residuals recorded by the r3an9y audit (BUG-961..971 landed 2026-09-08; these were NOT)
...
The BUG-964 copy rule already refuses copying such a struct.

### LOW — enum with no zero variant: CARRIER storage is guarded only at the switch
BUG-930 rejects a bare `E e;` and traps at an exhaustive switch. A struct field, array element,
...
BUG-963 makes a static local shared between ISR and main require `volatile`; the "volatile but
...
`H h; h.p = &v; h.q = &x; ThreadHandle th = spawn w(h);` — BUG-971's borrow root is single-
...
## CLOSED 2026-09-08 (BUG-961..971) — BRANCH `loving-davinci-r3an9y` (2026-09-04): 12 holes, 35 live negatives

**All twelve adopted by cherry-pick of `df4e51e` and renumbered BUG-961..971 (A was already
BUG-930). Verified live on main first, then closed-right against the merged compiler; the
residuals their audit recorded are the `## OPEN — residuals` entry below. Kept as the record.**
...
### ~~CLASS 12 — FUNCPTR FACTORY THROUGH switch / do-while (MEDIUM) — `pstdqk`~~ — **CLOSED 2026-09-08 as BUG-977 (both sinks, +4 REACH +2 ISR cells). DO NOT REDO.**
...
### ~~CLASS 13 — `@container` WHOLE-OBJECT / ARRAY ELEMENT (MEDIUM) — `ef9cao`~~ — **CLOSED 2026-09-08 as BUG-984 (three-valued provenance, both sinks). DO NOT REDO.**
