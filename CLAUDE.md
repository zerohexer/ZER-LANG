...
| **`volatile` STRIP ("does this store/pass drop the qualifier?")** | var-decl, assignment, call arg, return, spawn arg, struct-literal field | ONE predicate `volatile_strip_detected` (peels each side INDEPENDENTLY — a raw pointer-kind test is FALSE for `?*T`) + the **volatile-strip cells in `tests/test_hw_matrix.c`**, one per sink. Three sinks hand-rolled the test and one had none at all; the struct-literal form coalesced three MMIO stores into one under -O2 |
| **Root-ident walk (`&a.b[i].c` → the IDENT)** | every safety check that asks "which variable is this?" | ONE `expr_root_ident()` in `ast.h`. The hand-rolled form is TWO SEQUENTIAL loops (`while(FIELD)…; while(INDEX)…;`) which is WRONG for any ALTERNATING chain — it stops on a FIELD node, the caller's `== NODE_IDENT` test fails, and the check silently does not run. Written five times, wrong five times. NO auto-gate: grep for two adjacent same-variable while-loops |
| **Doc / compiler agreement** | every complete `.zer` example in `docs/reference.md` | `tools/audit_reference_examples.sh` (`make check-reference`) — checks BOTH directions: an example shown as working must compile, and one whose LIVE code says `COMPILE ERROR` must be rejected. Fragments are skipped WITH THE COUNT PRINTED so the coverage limit is visible |
| **Freestanding branch selection** | every `__STDC_HOSTED__` gate in the emitted preamble | ONE `_ZER_HOSTED` predicate + `tools/audit_freestanding.sh` (`make check-freestanding`). `__STDC_HOSTED__` is a COMPILER-FLAG property, not a target property; `-DZER_FREESTANDING` lets the user declare it. Test the flag ALONE, not just with `-ffreestanding` — under `-ffreestanding` the old and new predicates agree, so that combination scores a broken compiler as passing |
...
|---|---|---|---|
| `tests/zer/` | ZER integration tests (positive — must compile + run + exit 0) | 536 | `tests/test_zer.sh` |
| `tests/zer_fail/` | ZER negative tests (must fail to compile) | 592 | `tests/test_zer.sh` |
| `tests/zer_gaps/` | Gap reproducers — run with the expectation INVERTED (compile-clean = the gap holds) | 24 | `tests/test_zer.sh` |
...
| `tools/audit_*.sh`, `tools/sink_matrix.sh` | Class-kill gates (walker-default, fixed-buffer, type-dispatch, carrier-dispatch, emit, per-sink escape/UAF, **reference-example**, **freestanding**) | 8 | `make check` |

**Counts above are generated by `ls`, not remembered.** Regenerate rather than
trust them — every number in this file has drifted at least once.
