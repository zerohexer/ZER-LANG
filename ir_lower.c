...
static int lower_expr(LowerCtx *ctx, Node *expr) {
    ...
    /* Unreachable: the switch above is exhaustive over NodeKind and
     * -Werror=switch enforces that. Named so GCC does not fall off the end of a
     * non-void function returning a LOCAL ID — an uninitialised local id would
     * be silently read as a real slot. */
    return -1;
}
