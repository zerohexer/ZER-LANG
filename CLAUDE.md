...
| **Non-atomic RMW ("is this a read-modify-write on a shared global?")** | spawn scan + ISR walker + the main-checker compound site | ONE resolver `resolve_write_target_global` (sees through `*p`/`*gp` to the pointee) + `assign_reads_own_target` (a written-out `g = g + 1` is the same operation) + the **RMW FORM grid in `tests/test_hw_matrix.c`** (site x spelling). The SPELLING axis matters as much as the site axis: `g = @truncate(u32,g)+1`, `g = idfn(g)+1` and `g = maybe(g) orelse 0` were all accepted at BOTH sinks until BUG-841, because the underlying walker was an if-chain that returned "no" for unlisted node kinds. **BOTH SIDES of the assignment go through the SAME resolver** (BUG-981): the target side resolved `*p` to `g` while the value side searched for the NAME `g`, so `*p = *p + 1` / `counter = *p + 1` / a helper's `*p = @truncate(u32,*p)+1` were "plain stores" at all three sites — 10 of 22 spellings accepted. The value walk is now `expr_reads_place` (exhaustive, no-default) with `place_is_global` resolving deref/field/index/slice places through `resolve_write_target_global` |
...
| **Index-step aliasing ("which slot does `hs[k]` name, and which others may it be?")** | the alloc arm, the store hook, the load / read-back arms (FIELD_READ, INDEX_READ, orelse, passthrough), the ident UAF walk, the THREE free sinks (builtin, extern destructor, summary-freeing callee), the leak pass, the merge | ONE key discipline (BUG-1012): `ir_extract_compound_key` keys an index-STABLE local symbolically (`"[k].p"`), the wild `"[?].p"` covers the rest, and `ir_key_may_alias` is the ONE answer to "may these two keys be one slot". BUG-741 patched exactly ONE of these sites (`free(arr[k])`, outermost index only) and the other eight stayed open for three months — a store, a load, a read and a leak were each a separately silent hole. A free through ANY spelling of a slot must widen its aliasing siblings (`ir_index_free_siblings`); a read with no exact entry must be a VIEW of them. Gate: `tests/zer_fail/var_index_*.zer` × `tests/zer/var_index_*_ok.zer` (sink × spelling; the `_ok` side is what keeps the fill/free loop compiling) |
| **Two SPELLINGS of one operation ("does the assign form track like the var-decl form?")** | `x = pool.alloc()` vs `T x = pool.alloc()`; `b = a` vs `T b = a` for a `move struct` | ONE registration helper **`ir_register_alloc_result`** called by BOTH arms. The two spellings LOWER DIFFERENTLY — var-decl gives `IR_CALL` + `COPY`, assign gives ONE passthrough `%t = ASSIGN <NODE_ASSIGN>` in which **the target local is never any instruction's `dest_local`** — so every arm keying on `dest_local` silently saw a temp (BUG-933: UAF, double-free, leak and use-after-move all accepted). When you add tracking for an operation, write BOTH spellings as a test. Related: a walk over field types must include **ARRAY** as a carrier, and **exceeding a depth guard must return the CONSERVATIVE answer** — `ir_contains_move_struct_field_depth` returned "no move struct" past depth 32, so a 34-deep nest defeated tracking by being deep. Two more spellings of this class closed 2026-09-13: `h.p = alloc(T);` into a FIELD/INDEX/GLOBAL slot registered nothing while `h.p = alloc(T) orelse return;` was tracked (BUG-1000, `ir_register_alloc_result_compound`), and `defer g.v += 1;` / `1 => g.v = 5,` took NO shared lock while the brace forms one keyword away locked (BUG-1003, `lower_stmt_in_block` — gated by an emit-audit fingerprint) |
...
| **Enum-forge doors** ("can this conversion produce a non-variant?") | GUARDED: `@bitcast`, `@truncate`, `@saturate` (`@cast` verified NOT to be one). REJECTED at the mint (2026-09-13, BUG-982..985): `@inttoptr(*Enum-or-bool-carrying T, …)`, `@ptrcast(*bool, …)` (enum was BUG-928), and a type_id-less `@pun` into anything carrying a pointer/slice/funcptr/enum/bool/optional/handle | the three `tests/zer_trap/*_enum_forged_*.zer` + `tests/zer_fail/{inttoptr_enum_*,bool_mint_*,pun_forge_*}.zer`. Patched THREE times across three sessions before the door set was written down; each fix closed one door and left the siblings. A LOAD through a minted pointer is not a conversion, so no conversion guard sees it — that is why those doors are closed at the MINT, not guarded |
...
| Enum forging (a value outside the variant set) | Runtime variant guard at the point of forgery — TRACKED, not banned, so reading an enum from a register still works. The GUARDED doors are EXACTLY THREE: `@bitcast` (BUG-843), `@truncate` (BUG-891), `@saturate` (BUG-910). `@cast` is NOT a door — it requires a distinct typedef and cannot name a bare enum. Three POINTER-MINTING doors are closed at the mint instead (a load is not a conversion, so no guard can see it): `@inttoptr` to an enum/bool-carrying pointee (BUG-983), `@ptrcast` to `*bool` (BUG-985; enum was BUG-928), and a type_id-less `@pun` into a forgeable target (BUG-982). The guard recurses struct fields, optional payloads and array elements, and each door wires it at BOTH emitter dispatch paths. **Adding a new value-producing conversion? It is a fourth door — wire the guard.** |
