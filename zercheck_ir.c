 ...
 * Either explicitly named "free" OR bodyless void fn whose first param is a
 * pointer (`*opaque` or `*T`)
...
static bool ir_is_extern_free_call(ZerCheck *zc, Node *call) {
    ...
    /* Signature heuristic: bodyless fn taking a pointer first param.
...
/* The Gap-32 backward-compat wrapper `ir_classify_method_call(call)` (which
 * called the _ex form with a NULL Checker, skipping receiver-type validation)
 * was REMOVED 2026-08-19: every call site had been migrated to the _ex form,
 * so it was dead code that still looked like a sanctioned entry point.
 * CLAUDE.md's Stage-1 rule 4 said to deprecate it as the migration proceeded;
 * this completes that. Always call ir_classify_method_call_ex(c, call). */
