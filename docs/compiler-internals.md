...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **BUG-604 (2026-04-23): `emit_rewritten_node` NODE_BINARY missed shift + signed-div safety wrappers.** Same class as BUG-595–599 (AST→IR migration lost runtime wrappers) but for the preserved-expression path instead of the decomposed-IR path. Any expression that `ir_lower` keeps as a single `IR_ASSIGN <expr>` (array index write `arr[expr] = v`, struct field write `s.f = expr`, await cond, void return) goes through `emit_rewritten_node`, which used to emit raw `(a << n)` / `(a / b)` with no `_zer_shl` / `_zer_shr` / signed-overflow trap. Fix: add shift macro emission and signed-overflow trap for `/` and `%` in NODE_BINARY, just before the generic fallback. Checker still forces compile-time non-zero-divisor guard so runtime zero check is not needed. Regression guards in `tests/zer/shift_in_array_index_spec.zer`, `tests/zer/shift_rhs_array_write.zer`, `tests/zer_trap/signed_div_in_index_trap.zer`. Methodology: re-ran the AST→IR diff audit (CLAUDE.md "AST→IR Emission Diff Audit") with targeted reproducers for each preserved-expression context. Key lesson for fresh sessions: when a fix covers "every IR_BINOP", ask *what expressions ir_lower preserves un-decomposed* — those have NO IR_BINOP instruction and must be covered separately in `emit_rewritten_node`.
