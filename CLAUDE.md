...
| **Deref identity ("what does this value read out of `*p` refer to?")** | var-decl init, assignment/field-store, return, call ARG — x the operand FORM (ident / field / index / nested deref / call result) x the TYPE laundered (pointer-slice-opaque alias, Handle allocation, move ownership) | ONE type-driven predicate `deref_identity_launder` (checker.c) — it asks the TYPEMAP for the operand type instead of shape-matching, so a NEW expression kind cannot reopen it. Closing the three SINKS (BUG-781/782) left the FORM and TYPE axes wide open for two weeks: `*h.pp`, `**ppp`, `*factory()`, `Tok b = *p`, `Handle k = *p` and `free(*p)` were ALL still accepted. **Enumerate FORM x TYPE x SINK before calling this class closed.** Negatives: `tests/zer_fail/deref_identity_*.zer`; boundary: `tests/zer/deref_scalar_ok.zer` |
| **Atomic cell ("is this location touched both plainly and atomically?")** | the @atomic_* target site + the plain WRITE site + the plain READ sites (ident / field / INDEX) | key on the full ACCESS PATH via `build_expr_key_a` (`atomic_cell_path_target`), decide by a may-ALIAS cross product, record an unresolvable path (variable index) as `wild` so it aliases every cell under its root. A per-field key covered 2 of 5 forms (BUG-786) |
...
|---|---|---|---|
| `tests/zer/` | ZER integration tests (positive — must compile + run + exit 0) | 523 | `tests/test_zer.sh` |
| `tests/zer_fail/` | ZER negative tests (must fail to compile) | 545 | `tests/test_zer.sh` |
...
| `tests/zer_gaps/` | Gap reproducers — expectation INVERTED (compile-clean = the gap holds) | 26 | `tests/test_zer.sh` |
