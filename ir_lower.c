...
static int lower_expr(LowerCtx *ctx, Node *expr) {
    ...
    /* Unreachable: the switch above is exhaustive over NodeKind (no `default:`,
     * enforced by -Werror=switch + walker_default_audit). GCC still warns
     * without this, and falling out of a non-void function is UB — return the
     * "no value" sentinel rather than whatever happens to be in the register. */
    return -1;
}
...
/* `classify_builtin_call` was REMOVED 2026-08-19 — dead since Phase 8d.
 * ir_lower.c does NOT emit IR_POOL_ALLOC / IR_SLAB_FREE / … ; every builtin
 * method collapses to a generic IR_ASSIGN or IR_CALL, and the classification
 * that matters happens LATER, in zercheck_ir.c's `ir_classify_method_call_ex`
 * which inspects the AST expression carried on the instruction. The helper had
 * no callers and implied a lowering path that does not exist. Do not
 * resurrect it: see the "IR_POOL_ALLOC etc. are NEVER emitted" note in
 * CLAUDE.md's CFG-migration section before touching this area. */
