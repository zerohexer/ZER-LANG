...
| **Non-atomic RMW ("is this a read-modify-write on a shared global?")** | spawn scan + ISR walker + the main-checker compound site | ONE resolver `resolve_write_target_global` (sees through `*p`/`*gp` to the pointee) + `assign_reads_own_target` (a written-out `g = g + 1` is the same operation) + the **RMW FORM grid in `tests/test_hw_matrix.c`** (site x spelling). The SPELLING axis matters as much as the site axis: `g = @truncate(u32,g)+1`, `g = idfn(g)+1` and `g = maybe(g) orelse 0` were all accepted at BOTH sinks until BUG-841, because the underlying walker was an if-chain that returned "no" for unlisted node kinds. **BUG-974 added a spelling that is not answerable inside ONE assignment at all** — `u32 t = g; g = t + 1;` — closed with a NAME->GLOBAL value taint (`RmwTaintEnt`, one shape and one set of query helpers, two tables because the ISR sink fills its own while a body is checked and the spawn sink fills its own while a CALLEE body is walked). The grid caught the spawn half being missed on the first run |
...
| **Depth/recursion caps in a SAFETY predicate** | every `depth > N` in checker.c / zercheck_ir.c | the cap's return value IS a safety decision. For a predicate that turns tracking ON, returning `false` at the cap is ACCEPT-UNSAFE (BUG-933 cause 5: use-after-move accepted at 34 levels of nesting, caught at 33). A cap bounds recursion; it must not decide safety. Audit each one for its DIRECTION, not its existence |
| **Bounds VERDICT of a loop counter ("MAY it index past the end, or WILL it?")** | the fixed-array index sink under a `for` / `while` / `do-while` driver | a VRP range is a MAY-hold fact and can only WARN + auto-guard (a straddling range can be safe). A counted loop with constant init/bound/step and a straight-line body gives the WILL-hold fact (`Checker.cert_loop_*`, BUG-958) that licenses a compile ERROR. Every driver kind must establish it or the off-by-N loop is a silent early return on that kind — the do-while got it through the while half; a new loop form needs its own arm |
| **Rendering a `double` into emitted C** | every float-literal emission site (was FIVE) | ONE helper `emit_double_lit` + **`tools/audit_float_literal.sh`** (BUG-966). `%.17g` is exact for FINITE doubles and prints the non-C tokens `inf`/`-inf`/`nan` for the other three, so `f64 x = 1e400;` emitted `x = inf;` and GCC blamed the user's `.zer` line. The gate fails the build on any float conversion specifier outside the helper — verified RED by reverting one site |
| **An intrinsic's ADVERTISED runtime check** ("does the trap the docs promise actually get emitted for THESE operand types?") | `@pun` type_id (BUG-969), the `*opaque` ERASURE that feeds it (BUG-971), `@inttoptr` enum reads (BUG-970), `@container` provenance (BUG-968) | NO auto-gate — a checklist question. For each intrinsic that promises a runtime check, ask **for which operand types is the emitted check a compile-time constant, or absent entirely?** `@pun`'s was `type_id != TGT && type_id != 0` where only struct/enum/union carry an id, so for every primitive pointee it folded to false and an integer became a working pointer with no `@inttoptr` and no `mmio`. Rejected only when the check cannot fire, the pointee types differ, AND the target carries a validity invariant |
| **A sentinel with two meanings** | `type_id == 0` = "no id for this kind" AND "unknown origin, do not check" (BUG-971) | NO auto-gate. When a value has a reserved sentinel, write down which question it answers. Twelve producer sites meant the first; the single consumer read the second, and the gap was a silent OOB. ONE producer now: `zer_pointee_tid` (the erasure id, reserved range for non-aggregates) vs `zer_pointee_tid_agg` (the direct-`@pun` aggregate-only id) — two questions, two functions |
...
| Emitter dual dispatch (AST ~3xxx + IR ~7xxx) | every intrinsic / coercion / safety-wrapper | `grep -n '"name"' emitter.c` MUST show TWO hits; the AST→IR emission diff audit. **A COERCION is in this class too, and its axis is the STATEMENT FORM** (BUG-986): `u32 r = f(arr)` lowers to `IR_CALL` (args decomposed, coerced), `r = f(arr)` lowers to `IR_ASSIGN <expr>` and lands in `emit_rewritten_node`, whose arg loop emitted raw. Test a new coercion at BOTH statement forms, not just var-decl init. The same commit found 8 sibling sites where `emit_array_as_slice` (which ends in `emit_expr`) sat in an if/else opposite `emit_rewritten_node(…, func)` — **one node, two emitters.** A shared helper that hardcodes ONE emitter is the shape to grep for |
...
| **MMIO const address ("what address does this `@inttoptr` designate?")** | the `@inttoptr` range+alignment gate, the direct `@inttoptr(...)[N]` bound, the local var-decl `mmio_bound`, the global var-decl `mmio_bound` | ONE query **`mmio_const_addr`** (BUG-944) — folds a literal, a `const` ident, and arithmetic over them. Before it, all four called plain `eval_const_expr`, so a NAMED base (the only spelling real firmware uses) derived no bound and deferred range/alignment errors to a runtime trap on first boot |
| **Documented examples** ("does the reference still compile — and is it still REJECTED where it says it is?") | every ```zer block in `docs/reference.md`, in BOTH directions | `tools/audit_reference_examples.sh` + `tools/reference_example_baseline.txt` — IN `make check` (the 8th gate). A block illustrating a rejection carries `<!-- audit: expect-error: <substring> -->` (BUG-945) and is asserted like a `zer_fail` negative; an undirected error block is SKIPPED, i.e. checked in neither direction. Twice the docs asserted a feature the parser did not have (funcptr arrays, bit-extract through a pointer), and once a rejection the compiler no longer performed |
...
**A GATE MUST BE POINTABLE AT A PRE-FIX COMPILER, or "verify it fires" measures nothing
(2026-08-28, adopted 2026-09-06 as BUG-975).** All ten `tests/test_*_matrix.c` grids hardcoded `./zerc`
in `find_zerc` and IGNORED argv, so `./test_hw_matrix /path/to/prefix/zerc` silently graded the
CURRENT compiler and reported the same all-green for both. That turns the one step that proves a
new cell is a net rather than a script into a no-op. They now honour **`ZER_MATRIX_ZERC`**, the
same affordance `tools/sink_matrix.sh` has always had as its first argument. Build a pre-fix
compiler with `git archive <sha> | tar -x -C $S` + `make zerc`, then
`ZER_MATRIX_ZERC=$S/zerc ./test_X_matrix` — the new cells must FAIL there.
...
and printed OK on a compiler that HAD a breach (BUG-969). Its oracle is GCC's
`-Werror=int-conversion` over an integer placed in each argument POSITION, and BUG-969 puts no
...
which found a heap-use-after-free in `zercheck_ir.c` at three sites (BUG-989) plus signed overflow
in `emitter.c` (BUG-990) on its FIRST run. That class is invisible to every .zer test by
...
| Silent truncation | Must `@truncate` or `@saturate` explicitly. The COMPOUND spelling follows the plain one: `a += f` (int += float, or float += int) is rejected like `a = f` (BUG-1002 — it used to be a raw C conversion that bypassed the saturation above) |
...
| Enum forging (a value outside the variant set) | Runtime variant guard at the point of forgery — TRACKED, not banned, so reading an enum from a register still works (as `@bitcast(State, *reg)`). The GUARDED doors are EXACTLY THREE: `@bitcast` (BUG-843), `@truncate` (BUG-891), `@saturate` (BUG-910); `@try_enum(E, x) -> ?E` (BUG-929) is the checked route. Everything else that could mint a non-variant is REJECTED at compile time: an integer literal at all ten value-flow sinks (BUG-927), enum arithmetic and `@ptrcast` (BUG-928), a bare declaration of an enum with no zero variant (BUG-930), `@pun` to an enum/bool/funcptr/pointer/slice when its type_id check cannot fire (BUG-969), `@inttoptr` to an enum-carrying pointee (BUG-970 — route it through `@inttoptr(*u32)` + `@bitcast`), and the BOOL siblings of both pointer-minting doors — `@ptrcast(*bool, u8ptr)` / `@inttoptr(*bool, addr)` (BUG-1001; a minted `*bool` loads a byte that is neither `true` nor `false`). Storage no rule can see (a zeroed struct field, a pool slot, cinclude memory) is caught at the CONSUMER: an exhaustive enum switch traps on a non-variant instead of running its last arm (BUG-950); a union switch traps on a tag outside 0..N-1 (BUG-976); a bool switch traps on a byte that is neither 0 nor 1 (BUG-983 — `bool` is a two-variant enum in everything but spelling, and `@bitcast(bool, n)` is guarded like the enum door). The guard recurses struct fields, optional payloads and array elements, and each door wires it at BOTH emitter dispatch paths. **Adding a new value-producing conversion? It is another door — wire the guard or reject.** |
...
| Deadlock | Same-statement multi-shared-type access → compile error. Emitter does lock-per-statement (no nested locks), so cross-statement ordering is safe. Only same-statement access to 2+ shared types is a real deadlock risk. A callee's transitive shared types count ONLY when the statement itself accesses a shared struct directly (it holds a lock around the call); a bare `r = f();` holds nothing (relaxed 2026-09-05). A `shared(rw)` root is a NON-re-entrant rwlock: the same type in the statement's direct set AND its callee-transitive set is rejected (BUG-998), and so is any call through a FUNCTION POINTER under an rw lock (BUG-1003 — the callee is unresolvable; the plain-`shared` callback idiom `go.cb()` stays legal) |
...
- `return/break/continue/goto in defer` — emission impossibility (corrupts cleanup flow). Same nested-loop exception; and since BUG-995 a value `orelse` inside a defer body is fine (the body is ordinary IR)
...
- `lower_auto_guards` (ir_lower.c) — bounds/UAF guard LOWERING (an IR branch since BUG-994; the emitter has no guard code)
- `ast_clone` (ast.c) — deep clone used to lower a defer body at each fire site (BUG-995); a new node kind with children must be cloned here
