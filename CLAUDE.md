...
| IR Phase 1-8 (data structures, lowering, 3AC, emission, zercheck, VRP) | Done | Done. Migration: --use-ir, **195/195 ZER + 761/761 rust, 0 fail, 0 hang**. Phase 8: (a) on-demand locals, ident rewriting, scope conflict BUG-507, async yield BUG-508/509/510. (b) `lower_expr()` wired for ident/literal/binary/unary/field/index/string → IR_COPY + IR_BINOP/UNOP/FIELD_READ emit from local IDs. (c) `can_lower_expr()` conservative guard — conditions/returns decomposed via `lower_expr` when safe. ZERO `emit_expr` in `emit_ir_inst`. 17 `emit_ir_value` calls remain as fallback for complex expressions (opaque, builtins, handle deref, intrinsics, bounds checks). Each needs 50-400 lines to replace. `emit_ir_value(e, expr)` delegates to `emit_expr` — correct because ident names are rewritten to IR local names. |
