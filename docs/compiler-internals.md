...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **Verification needed:** run ALL test suites (module 28, rust 786, zig 36, cinclude) with `--use-ir` before flipping default. cinclude patterns (`*opaque` + C interop) untested on IR path.
  - **195/195 ZER --use-ir, 786/786 rust, make check green (AST default).**
