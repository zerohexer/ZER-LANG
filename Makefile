...
# Global-initializer constant-context oracle (2026-08-20). A file-scope
# initializer must be a C constant expression; the checker's list of intrinsics
# that cannot be one was maintained BY HAND and had drifted twice (G7 added six
# names; a 2026-08-20 sweep found eleven more still accepted by the checker and
# stopped only by GCC — `zerc -o out.c` exited 0 for a program that cannot be
# compiled). This grid DERIVES the intrinsic set from the compiler at run time
# and asserts the checker's verdict agrees with GCC's, so a new intrinsic is
# covered the day it lands. Verified to fire: 7 failures before the fix, 0 after.
test_global_init_matrix: tests/test_global_init_matrix.c
	$(CC) $(CFLAGS) -o $@ $<
