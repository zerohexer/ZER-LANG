...
| ~~Q4~~ **DONE 2026-08-23 (BUG-864)** | `@saturate` not total (float bounds rounded UP; AST signed-64 arm had no clamp) | Both halves confirmed live and fixed. Measured before: `@saturate(i32, 2147483648.0f)` = **-2147483648**, `@saturate(u32, 4294967296.0f)` = **0**, `@saturate(i64, 2^63)` = INT64_MIN, `@saturate(u64, 2^64)` = 0 — the clamp intrinsic WRAPPED at the one boundary that matters, silently. A float source now routes through the same total `_zer_f2i`/`_zer_f2u` helper that plain casts and `@truncate` use (exact power-of-two bounds, NaN = 0), and the AST signed-64 arm got the clamp it never had. `tests/zer/saturate_total.zer` (25 assertions, verified to COMPILE and FAIL on a pre-fix build) |
