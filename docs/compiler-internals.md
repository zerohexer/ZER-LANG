...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **3 `emit_expr` calls remain** (from original 17): 2 in IR_ASSIGN (complex expr passthrough + void expr), 1 in IR_CALL (builtins). These handle ~1800 lines of emit_expr logic: NODE_ASSIGN (field/compound/index writes), NODE_CALL (builtins), NODE_FIELD (handle auto-deref, slice .len), NODE_INTRINSIC, NODE_INDEX (bounds), NODE_IDENT (globals), NODE_BINARY (opaque .ptr), NODE_SLICE. Eliminating these 3 requires extracting/replicating that logic into dedicated IR emission functions.
