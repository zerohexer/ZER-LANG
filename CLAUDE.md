...
| **VRP condition NARROWING ("what does `var CMP const` tell me?")** | then-branch, guard-inverse, else-branch, short-circuit RHS | **KILLED (BUG-920): ONE `vrp_parse_cmp` + `vrp_push_cmp_range`, four callers.** It was four hand-written switch tables (holds/inverse x operand order) and a new caller would have been a fifth and sixth — so a new narrowing site is a CALL, never a new table. Two invariants the callers rely on: dropping a narrowing by truncating `var_range_count` is sound ONLY because an assignment unions into every live entry (BUG-913) and `&x` marks them all (BUG-918); and an EMPTY intersection must record the FULL range, since an inverted `[9,3]` reads as PROVEN_SAFE at `index_range_verdict` and as nonzero at four division sites |
...
| **Deferred whole-program pass ("did this pass see the WHOLE program?")** | every pass in `checker_post_passes_multi` — and, per pass, every MODULE | the `test_modules/xmod_post_*` pairs. The N here is not sites in the source, it is **FILES**: a pass driven by one `file_node` is silently exempt for every import, and moving the identical code into a module made five rules stop firing (BUG-921). Split per-FILE collect from whole-program analyse — stack chains and resource-init must be collected from every module BEFORE either is judged, or per-module runs both miss (a chain that crosses a boundary) and FALSE-POSITIVE (a resource initialised from another module). Any new deferred pass goes in the right half AND gets an `xmod_post_*` cell |
...
- **Bounds safety philosophy: prove, don't guard.** When array index can't be proven safe, prefer extending range propagation to make it provable (zero overhead) over adding auto-guard/bounds-check (runtime overhead). `arr[func()]` with proven return range = zero overhead. Auto-guard is the fallback, not the goal. For loop conditions (while/for), auto-guard can't work (value changes each iteration) — instead extend `find_return_range` to prove more call chains. Inline `_zer_bounds_check` trap is correct for genuinely unprovable cases (external input).
  **CONDITION NARROWING IS ONE HELPER — `vrp_parse_cmp` + `vrp_push_cmp_range` (BUG-920).**
  "What does `var CMP const` tell me about `var`?" used to be FOUR hand-written switch tables
  in the NODE_IF handler; a third caller would have been a fifth and sixth. It is now one
  normalising parser plus one range table, called from the then-branch, the guard-inverse, the
  else-branch and the short-circuit RHS. **A new narrowing site is a CALL, never a new table.**
  Two soundness facts the callers rely on and must not be broken: the narrowing is dropped by
  truncating `var_range_count` (sound ONLY because an assignment unions into every live entry,
  BUG-913, and `&x` marks them all address-taken, BUG-918 — the three fixes are one mechanism),
  and an EMPTY intersection records the FULL range, not the inverted one (an inverted `[9,3]`
  reads as PROVEN_SAFE at `index_range_verdict` and as nonzero at four division sites).
