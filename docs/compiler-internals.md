...
### Cast EMISSION is one function, not three (BUG-979, 2026-09-02)
...
## Audit 2026-09-04 — the mechanisms behind BUG-961..919 (read before touching any of them)
...
**Capture locals dedup only with capture locals (`ir_add_local`, BUG-961).** The dedup key
...
**Per-statement lock collector covers NODE_SPAWN per ARGUMENT (BUG-962).** The emitter locks
...
**Static locals in the race scans (BUG-963).** Spawn: `scan_unsafe_global_access` keeps a
...
**Unique-resource copies (BUG-964).** `type_unique_resource_name` (Pool/Ring/Slab/Arena/
...
**Enum non-variant guard (BUG-930).** `ir_lower` keeps the elided last-arm entry of an
...
**Zero value of a return type (BUG-965).** `emit_return_null` delegates to `emit_zero_value`,
...
**View-root resolution (BUG-966).** `ir_view_root_handle(zc, func, ps, expr)` in zercheck_ir.c
...
### BUG-967..924 — the second half of the 2026-09-04 audit (mechanisms)

- **Slice header is read-only (BUG-967).** `slice_header_field(c, n)` names `.ptr`/`.len` on a
...
- **Parser lookahead for `IDENT ( *` (BUG-968).** The C-style funcptr declaration is committed
  only on `IDENT ( * IDENT ) (`; anything else is an expression statement. `f(*p);` is a call.
- **Packed array-field views (BUG-969).** `packed_array_field_view(c, v)` = NODE_FIELD whose type
...
- **uN bit-slice store (BUG-970).** `emit_intn_mask` wraps VALUES (IR_BINOP/IR_UNOP); the
...
- **Borrow root (BUG-971) — the scoped-spawn sink's missing variable.** `Symbol.borrow_root_name`
