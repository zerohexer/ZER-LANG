...
| VRP range JOIN (merge at a control-flow join) | NODE_IF (§C#13 ✅), switch-arm, for-body, while/do-while body, do-while first-iter, goto/label — **and the loop CONDITION itself** (BUG-1015: it is evaluated per iteration under the loop-carried range, and all three drivers checked it under the pre-loop range — a silent OOB read in the CVE demo's own parse loop) | mirror `vrp_snap_take/restore/join` per node-kind; a missing kind = silent OOB. NO auto-gate — checklist every control-flow kind, and every POSITION a loop evaluates (init / cond / step / body) |
