...
| IR Phase 1-8 (data structures, lowering, 3AC, emission, zercheck, VRP) | Done | Done. Migration: --use-ir, **195/195 ZER, 0 fail, 0 hang**. Phase 8: (a) on-demand locals, ident rewriting, scope conflict BUG-507, async yield BUG-508/509/510. (b) `lower_expr()` wired for ident/literal/binary/unary/field/index/string → IR_COPY + 3AC ops emit from local IDs. (c) **`can_lower_expr()` DELETED, `emit_ir_value()` DELETED (Phase 8d).** `lower_expr` is unconditional — ALL expressions get local IDs. Complex expressions (calls, intrinsics, builtins, orelse, casts, struct-init) passthrough via `IR_ASSIGN{dest,expr}` → emitter calls `emit_expr` directly. All other IR ops (BRANCH, RETURN, CALL, AWAIT, BINOP, UNOP, FIELD_READ, INDEX_READ, builtins, INTRINSIC) use local IDs exclusively via `emit_local_name()` helper. Zero `emit_ir_value` calls. BUG-513 through BUG-522. **`emit_expr` in `emit_ir_inst`: ZERO.** New `emit_rewritten_node` function handles rewritten AST directly for: ident, literals, binary (opaque .ptr), unary, field (enum/slice/array/pointer/Handle auto-deref), call (with builtin detection), typecast, struct_init, 13 intrinsics (@size/@truncate/@saturate/@bitcast/@cast/@offset/@ptrtoint/@trap/@ptrcast/@inttoptr/@container), slice, orelse (IR branches). `emit_rewritten_node` delegates to `emit_expr` for 6 remaining patterns: builtins, opaque field, NODE_ASSIGN, NODE_INDEX, complex intrinsics, default. 162/195 passing on --use-ir, 33 edge cases remain. |
