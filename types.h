...
struct Symbol {
    ...
    struct {
         ...
         * per-site checks (NODE_SPAWN, slab.alloc, NODE_YIELD inside @critical
         * via the matching context flag) already fired. Without this split,
         * `interrupt USART1 { slab.alloc(); }` produces TWO errors for the
         * same root cause — one at the alloc site and one at the interrupt
         * line. With it, only the per-site (more actionable) error fires. */
