...
(**`keep_value_roots`**, BUG-1353 — the ONE "which non-keep params may this VALUE carry?"
...
| **What a CALLEE STORES of its params ("where did the argument go?")** | a field chain, an ELEMENT (literal / variable index / container push), the WHOLE object (`*s =`), an aggregate VALUE carrying the param, a GLOBAL (bare / element / literal) — crossed with the argument (reference vs aggregate) | ONE collector **`ir_ps_scan`** → `FuncSummary.param_store` (dst -1 = a global by its full key; `[*]` = an unnameable index) + ONE applier `ir_apply_param_stores` (BUG-1351/1352; BUG-1241 had recorded field chains only). Gate: **SHAPE p50 in `tools/sink_matrix.sh`**. A new store spelling is a case in the scan, never a second collector |
| **A call argument that is ITSELF a call result** (`id(id(a))`, `unwrap(wrap(a))`, `wrap(a).p`) | the view query, the aliased-argument query, the funcptr barrier, the field-free widening, the free sink | ONE resolver **`ir_resolve_returned_arg`** (BUG-1350) — the inner call is a temp in the IR but the sinks read the ORIGINAL AST, where it had no key. Gate: **SHAPE p49** |
| **Keep inference: which non-keep params does this VALUE carry?** | var-decl init, assignment (incl. `*h =`), if/switch capture, the persist sink, spawn arg, keep-call edges | ONE query **`keep_value_roots`** (a bitmask; `Symbol.nonkeep_root_mask`) + ONE taint `taint_nonkeep_from_value` (BUG-1353 — the alias sites stopped at any call / literal / orelse). Gate: **SHAPE p51** |
