...
Call sites: zercheck.c `is_handle_invalid` + `is_handle_consumed`, zercheck_ir.c `ir_is_invalid`, checker.c Pool/Ring count + control-flow statement handlers + return-escape check + @ptrcast provenance check, types.c `type_is_integer/signed/unsigned/float/numeric` + `can_implicit_coerce` (all delegate).
...
2. Phase 1 — 40 pure predicates (~100 hrs) — **31/44 done (70%)**
