...
static void emit_rewritten_node(Emitter *e, Node *node, IRFunc *func) {
    ...
    case NODE_BINARY: {
         ...
         * shift handling. */
        if (node->binary.op == TOK_LSHIFT || node->binary.op == TOK_RSHIFT) {
            /* G3-shift (2026-08-09): a uN/iN (or narrow-native) shift RESULT must
             * be width-wrapped to N bits, exactly like +,-,*,&,|,^ below and the
             * IR path (emit_intn_mask after _zer_shl at IR_BINOP). _zer_shl/_zer_shr
             * only guard shift-by->=CARRIER-width; they do NOT mask the result to N,
             * so `u3 a=3; u32 x; x = a << 2;` gave 12 instead of 4, and an iN shift
             * kept the wrong SIGN. This AST-passthrough store path was the only one
             * missing the mask (var-decl/return/call-arg lower to IR_BINOP). The mask
             * on a right shift is a no-op for the value but keeps iN sign-extension
             * correct. */
