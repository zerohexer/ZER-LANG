...
typedef LabelInfo GotoInfo;

static void *copy_arm_path(Checker *c, void **stack, int depth) {
...
static bool if_has_capture(Node *if_node) {
    return if_node && if_node->if_stmt.capture_name != NULL;
}
/* Does the switch arm bind a capture? */
static bool switch_arm_has_capture(SwitchArm *arm) {
...
static void collect_labels(Checker *c, Node *node, LabelInfo *labels,
    ...
    case NODE_LABEL:
        if (*count < max) {
            ...
            labels[*count].arm_path = (void **)copy_arm_path(c, arms->stack, arms->depth);
    ...
    case NODE_BLOCK:
        ...
        break;
    case NODE_IF: {
        bool has_cap = if_has_capture(node);
    ...
    case NODE_SWITCH:
        for (int i = 0; i < node->switch_stmt.arm_count; i++) {
            SwitchArm *arm = &node->switch_stmt.arms[i];
            bool has_cap = switch_arm_has_capture(arm);
