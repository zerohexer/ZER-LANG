...
| **Enum-forge doors** ("can this conversion produce a non-variant?") | `@bitcast`, `@truncate`, `@saturate` — and `@cast` verified NOT to be one; `@pun` to a primitive-pointee (BUG-987) and `@inttoptr` to an enum-carrying pointee (BUG-988) are REJECTED so the guarded set stays at three | the three `tests/zer_trap/*_enum_forged_*.zer` + `pun_forge_enum.zer` / `inttoptr_enum_*.zer`. Patched THREE times across three sessions before the door set was written down; each fix closed one door and left the siblings |
