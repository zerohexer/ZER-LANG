...
int main(void) {
    ...
    printf("[bounds check — PROVABLY-OOB variable index rejected (2026-08-16)]\n");
     ...
     * the test had codified a silent gap. VRP knows idx is exactly 10 and the
     * array holds 4, so the access can never be in bounds; falling into the
     * auto-guard meant no compile error AND no runtime trap, just an early
     * `return` that skipped `arr[idx] = 99` and the `return arr[0]` behind it
     * (observably: 0 instead of 42). The literal form `arr[10]` above has always
     * been a hard error; this is its ident sibling. */
    {
        tests_run++;
        Arena a; arena_init(&a, 128*1024);
        if (!zer_to_c(
