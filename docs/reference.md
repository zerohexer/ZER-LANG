...
inside a `defer` body. (Before 2026-09-12 the assignment and `defer` forms
...
- The lock is emitted **per statement, in every statement position** — a function
  ...
  of a shared access that skips it. (Before 2026-09-12 a braceless switch arm,
  `0 => g.x = 5,`, did skip it and emitted an unsynchronized write; the braced
  form of the same line locked correctly. See BUGS-FIXED BUG-981, and
...
- A callback the spawn target invokes is scanned too — if it touches a
  ...
  `@once` body. (Before 2026-09-12 the `switch`-arm and `do-while` positions
