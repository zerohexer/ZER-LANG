...
static int lower_expr(LowerCtx *ctx, Node *expr) {
    ...
    /* Unreachable: the switch above enumerates every NodeKind with no default:,
     * which -Werror=switch enforces. GCC still cannot prove an enum-typed value
     * is in range, so without this the function FALLS OFF THE END and returns an
     * indeterminate int — which the caller uses as a LOCAL ID. A garbage local id
     * indexes func->locals[] out of bounds, i.e. exactly the layout-fragile
     * corruption that reads "defined" to every sanitizer.
     *
     * Reaching here means expr->kind holds a value outside NodeKind: memory
     * corruption or a node built by hand with an unset kind, never a language
     * feature. Follow the emitter's ir_validate convention and fail loudly rather
     * than lower something we cannot describe. */
    fprintf(stderr,
            "INTERNAL ERROR: lower_expr reached an out-of-range NodeKind (%d) "
            "at line %d — please report with a minimal reproducer.\n",
            (int)expr->kind, expr->loc.line);
    abort();
}
