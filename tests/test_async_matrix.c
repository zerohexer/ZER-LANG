...
static int run_neg(const char *name, const char *code) {
    total++;
    FILE *f = fopen(ZT("/tmp/_zer_as.zer"), "w");
    if (!f) { fprintf(stderr, "cannot create temp file\n"); return 0; }
    fputs(code, f); fclose(f);
    char cmd[512];
    snprintf(cmd, sizeof(cmd), ZT("%s /tmp/_zer_as.zer -o /tmp/_zer_as.c 2>/tmp/_zer_as.err"), zerc_path);
    if (system(cmd) == 0) {
        failed++; false_neg++;
    ...
    char eb[4096]; eb[0] = 0;
    FILE *e = fopen(ZT("/tmp/_zer_as.err"), "r");
