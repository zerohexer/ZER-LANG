...
static void ir_check_expr_uaf(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    case NODE_CALL:
        ...
        if (expr->call.callee && expr->call.callee->kind == NODE_FIELD) {
            ...
            /* BUG-1017: an INDEXED callee — `cbs[0](1)` on a heap slice of
    ...
    case NODE_CALL:
        ...
        for (int i = 0; i < expr->call.arg_count; i++)
            ir_check_expr_wrong_pool(zc, func, ps, expr->call.args[i],
                                     line, rs);
        /* BUG-1017: mirror the UAF walker — an indexed callee is a read of
...
static void ir_check_inst(ZerCheck *zc, IRPathState *ps, IRInst *inst, IRFunc *func) {
    ...
    case IR_NOP: {
        ...
        /* BUG-1017 (2026-09-14): a spawn's arguments are PARENT-evaluated
    ...
    case IR_COPY: {
        ...
        IRHandleInfo *src_h = ir_find_handle(ps, inst->src1_local);
        /* BUG-1018 (2026-09-14): a POINTER/slice/opaque/Handle PARAM copied into a
    ...
    /* BUG-1017 (2026-09-14): an AWAIT condition is re-evaluated on every poll
