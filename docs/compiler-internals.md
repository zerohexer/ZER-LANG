...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **Architecture after Phase 9:** Decomposed (zero emit_expr): ident locals, literals, binary (simple), unary, field (simple struct/pointer), index (simple), typecast (3-path), struct_init (compound literal), simple calls (from local IDs), orelse (branches), captures (IR_COPY), ?void hoist (IR_LITERAL). Passthrough (uses emit_expr): builtins (pool/slab/ring/arena inline C), intrinsics (@ptrcast/@size), complex field (handle auto-deref), complex index (bounds check), complex binary (opaque), assignments (field/compound writes), globals/enums, slices.
