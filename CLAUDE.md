...
| **Bounded-index VERDICT ("does this index's range settle the access?")** | `array[lit]`, `array[ident]`, `array[call()]`, `mmio[lit]`, `mmio[ident]` | ONE query `index_range_verdict` (IN_BOUNDS / ALWAYS_OOB / UNKNOWN) + the **BOUNDS GRID in `tests/test_bounds_matrix.c`** (sink x range class, no-`default:` enums). The grid asserts WHICH VERDICT is reached, not just that a diagnostic exists — two of the three failure modes compile and run, so an exit-code probe scores them "fine". A missing ALWAYS-OOB arm is SILENT AT BOTH ENDS: the auto-guard's runtime form is `if (i >= N) { return <zero>; }`, i.e. an early return, not a trap |
