...
| Emitter dual dispatch (AST ~3xxx + IR ~7xxx) | every intrinsic / coercion / safety-wrapper | `grep -n '"name"' emitter.c` MUST show TWO hits; the AST→IR emission diff audit. **A COERCION is in this class too, and its axis is the STATEMENT FORM** (BUG-924): `u32 r = f(arr)` lowers to `IR_CALL` (args decomposed, coerced), `r = f(arr)` lowers to `IR_ASSIGN <expr>` and lands in `emit_rewritten_node`, whose arg loop emitted raw. Test a new coercion at BOTH statement forms, not just var-decl init. The same commit found 8 sibling sites where `emit_array_as_slice` (which ends in `emit_expr`) sat in an if/else opposite `emit_rewritten_node(…, func)` — **one node, two emitters.** A shared helper that hardcodes ONE emitter is the shape to grep for |
