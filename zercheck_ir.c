...
typedef struct {
    ...
    int freed_by_origin;   /* BUG-995: defer_origin of the instruction that freed it (0 = explicit) */
...
static bool ir_use_guard_disjoint(ZerCheck *zc, IRHandleInfo *h) {
    if (!h) return false;
    /* BUG-995: a use or free by the SAME defer registration that already freed
...
/* BUG-979: alloc_id 0 is the "no allocation identity / untracked" SENTINEL —
...
static IRPathState ir_merge_states(IRPathState *states, int state_count) {
    ...
    for (int si = first_live + 1; si < state_count; si++) {
        ...
        for (int hi = 0; hi < result.handle_count; hi++) {
            ...
            /* BUG-995: carry the freeing defer registration alongside free_line
            ...
            /* BUG-981: UNION the view sets. `view_alloc_ids` records the
...
/* BUG-981: fill `dh`'s VIEW SET from a multi-view call result.
 *
...
/* BUG-952: the tracked ALLOCATION an lvalue path lives in, or NULL.
 *
...
static bool ir_register_alloc_result(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    if (mc == IRMC_ALLOC || mc == IRMC_ALLOC_PTR) {
        ...
        if (h) {
            ...
            h->state = IR_HS_ALIVE;
            h->alloc_line = line;
            h->alloc_id = ir_alloc_id_of_local(dest);   /* BUG-979: never 0 */
...
static void ir_propagate_alias_state(IRPathState *ps, IRHandleInfo *target,
    ...
    for (int i = 0; i < ps->handle_count; i++) {
        ...
        if (h->alloc_id == aid && !ir_is_invalid(h)) {
            h->state = new_state;
            h->free_line = line;
            h->freed_by_origin = target->freed_by_origin;   /* BUG-995 */
        }
    }
}
...
static void ir_check_inst(ZerCheck *zc, IRPathState *ps, IRInst *inst, IRFunc *func) {
    zc->cur_defer_origin = inst->defer_origin;   /* BUG-995 */
    ...
    case IR_SLAB_FREE_PTR: {
        ...
        if (h) {
            if (ir_use_guard_disjoint(zc, h) && h->state == IR_HS_FREED) {
                /* BUG-995: same-registration re-fire — idempotent. */
            ...
            h->freed_by_origin = zc->cur_defer_origin;   /* BUG-995 */
            ...
            if (fk == TYPE_POINTER || fk == TYPE_SLICE || fk == TYPE_OPAQUE) {
                ...
                if (ir_extract_compound_key(zc, func, inst->expr->field.object,
                    ...
                    if (fh && fh->alloc_id != 0) {
                        ...
                        /* BUG-989: `fsnap.state`, NOT `fh->state`. ir_add_handle may
                        ...
                        if (ir_extract_compound_key(zc, func, inst->expr,
                            ...
                            if (wh) {
                                ...
                                /* BUG-989, the instance ASan actually caught: read the
         ...
         * subslice of a param / stack slice (no handle) is unaffected. */
        {
            ...
            if (slice_val && b_dest >= 0) {
                ...
                if (sroot && sroot->kind == NODE_IDENT) {
                    ...
                    /* BUG-952: the walk stopped at a FIELD (`h.arr[0..]`,
        ...
        if (inst->expr && inst->expr->kind == NODE_ASSIGN &&
            ...
            if (rv && rv->kind == NODE_CALL && rv->call.callee &&
                ...
                if (cs && cs->returns_param_color > 0) {
                    ...
                    /* BUG-981: the MULTI-VIEW sibling of the arm above. It was
        ...
        if (inst->dest_local >= 0 && inst->expr) {
            ...
            if (rhs_pv && rhs_pv->kind == NODE_UNARY && rhs_pv->unary.op == TOK_AMP) {
                ...
                if (target && target->kind == NODE_IDENT && !used_compound) {
                    ...
                    /* BUG-952: `&pool.get(h).v` / `&pool.get(h).arr[0]` — the root
        ...
        if (summary && summary->returns_param_color > 0 &&
            ...
            if (param_idx < inst->expr->call.arg_count) {
                ...
                if (arg_h) {
                     ...
                     *
                     * BUG-989: the snapshot must be taken BEFORE ir_add_handle,
    ...
    case IR_DEFER_FIRE:
        /* BUG-995 (2026-09-05): a defer body is LOWERED INLINE at every fire site
    ...
    case IR_STRUCT_INIT_DECOMP: {
        Node *si = inst->expr;
        /* BUG-980: J3 (above) registered the ALIAS but left this case the only
        ...
        if (si && si->kind == NODE_STRUCT_INIT && inst->dest_local >= 0 &&
            ...
            for (int i = 0; i < inst->call_arg_local_count; i++) {
                ...
                if (vloc < 0) continue;
                /* BUG-980 (move half): a move struct handed to a designated
...
/* BUG-995: is `pred -> bi` the SKIP edge of a defer-body gate? An inlined
 ...
 * merge — which is exactly the semantics the pre-BUG-995 analysis had when the
...
bool zercheck_ir(ZerCheck *zc, IRFunc *func) {
            ...
            if (bb->pred_count == 0) {
                ...
                if (bi > 0 && func->blocks[bi - 1].inst_count > 0) {
                    ...
                    if (prev_last->op == IR_RETURN) {
                        ...
                        /* BUG-995: this block is UNREACHABLE (no preds) — its own
    ...
    for (int bi = 0; bi < func->block_count; bi++) {
        ...
        if (bb->pred_count == 0) {
            if (bi > 0 && func->blocks[bi - 1].inst_count > 0) {
                ...
                if (prev_last->op == IR_RETURN) {
                    merged = ir_ps_copy(&block_states[bi - 1]);
                    merged.terminated = true;   /* BUG-995: dead block, see the fixpoint twin */
                } else {
                    ir_ps_init(&merged);
                }
            } else {
    ...
    /* BUG-995: the Phase C3 "apply every defer body's frees/uses to every
    ...
    for (int bi = 0; bi < func->block_count; bi++) {
        ...
        for (int hi = 0; hi < ps->handle_count; hi++) {
            ...
            /* BUG-982 (latent): the four report sites below index
