...
/* BUG-987: the runtime provenance id of a POINTER's pointee — ONE definition.
 *
...
static void emit_zero_value(Emitter *e, Type *t);   /* defined below (BUG-965) */
static void emit_return_null(Emitter *e) {
    ...
    if (eff->kind == TYPE_OPTIONAL && !is_null_sentinel(eff->optional.inner)) {
        ...
        /* BUG-965: was a bare `return 0;` for EVERY non-optional type, which is
...
static void emit_shared_ensure_init(Emitter *e, Node *root, const char *arrow) {
    ...
    /* BUG-989: the CHECKER's flag, which is set for every `@cond_*` in every
...
static void emit_zero_value(Emitter *e, Type *t) {
    ...
    if (inner->kind == TYPE_OPTIONAL && !is_null_sentinel(inner->optional.inner)) {
        ...
        /* BUG-965: a slice is a {ptr,len} struct too. A bare `0` here is what
...
/* BUG-980 (2026-09-02): a function whose RETURN TYPE is a function pointer needs
...
static void emit_expr(Emitter *e, Node *node) {
    ...
    case NODE_TYPECAST: {
        /* Site 1 of 3. All cast policy lives in emit_cast_value (BUG-979). */
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            if (tgt_eff && tgt_eff->kind == TYPE_POINTER &&
                ...
                uint32_t tid = zer_pointee_tid(src_eff);   /* BUG-987 */
                ...
                uint32_t expected_tid = zer_pointee_tid(tgt_eff);   /* BUG-987 */
                if (expected_tid > 0) {
                    int tmp = e->temp_count++;
            ...
            /* BUG-987: TWO ids, because @pun has two situations.
             *
            ...
            if (src_is_opaque) {
                 ...
                 * BUG-987: the DYNAMIC id here — the source's true origin is only
            ...
            /* BUG-930: synthesized by ir_lower at the last arm of an exhaustive
    ...
    if (node->struct_decl.is_shared) {
        if (node->struct_decl.is_shared_rw) {
            ...
            if (st && type_dispatch_kind(st) == TYPE_STRUCT)
                needs_condvar = st->struct_type.uses_condvar;   /* BUG-989 */
...
static void emit_func_decl(Emitter *e, Node *node) {
    ...
    /* BUG-980: the prototype path had NO funcptr-return handling at all, so a
...
/* BUG-989: `register_condvar_type` / `is_condvar_type` and the
...
static void prescan_spawn_in_node(Emitter *e, Node *node) {
    ...
    case NODE_EXPR_STMT:
        /* BUG-989: the condvar prescan that lived here is GONE. It matched a
...
/* --- the shared cast emitter declared beside classify_cast (BUG-979) --- */
static void emit_cast_operand(Emitter *e, const CastOperand *op) {
    switch (op->kind) {
...
static void emit_cast_value(Emitter *e, const CastOperand *op,
    ...
    case CASTF_TO_OPAQUE: {
        uint32_t tid = zer_pointee_tid(src_eff);        /* BUG-987: ONE id definition */
        emit(e, "(_zer_opaque){(void*)(");
    ...
    case CASTF_FROM_OPAQUE: {
        uint32_t expected = zer_pointee_tid(tgt_eff);   /* BUG-987 */
        if (expected > 0) {
            int tmp = e->temp_count++;
            emit(e, "({ _zer_opaque _zer_pc%d = ", tmp);
            emit_cast_operand(e, op);
...
static void emit_rewritten_node(Emitter *e, Node *node, IRFunc *func) {
    ...
    case NODE_ASSIGN: {
        ...
        /* BUG-970: a BIT-SLICE target (`r[11..8] = v` on a u12) has the uN type
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            /* BUG-930: the last arm of an exhaustive enum switch used to be
            ...
            if (src_is_opaque) {
                 ...
                 * BUG-987: the DYNAMIC id — see the AST sibling. */
                uint32_t tgt_tid = tgt_tid_dyn;
                if (tgt_tid > 0) {
                    int tmp = e->temp_count++;
                    emit(e, "({ _zer_opaque _zer_pn%d = ", tmp);
...
static void emit_ir_inst(Emitter *e, IRInst *inst, IRFunc *func) {
    ...
    case IR_CAST: {
        ...
        if (inst->dest_local >= 0 && inst->cast_type) {
            ...
            /* Site 3 of 3 — see emit_cast_value (BUG-979). The operand is a
