...
/* BUG-966: render a `double` as a VALID C floating constant.
 *
...
/* BUG-971: the runtime provenance id of a POINTER's pointee — ONE definition.
 *
 ...
 * rejects a widening pun (BH-18 #4) and a forging one (BUG-969), and DELIBERATELY
...
/* BUG-986: emit_array_as_slice must be usable from the IR-REWRITTEN path too,
...
static void emit_zero_value(Emitter *e, Type *t);   /* defined below (BUG-951) */
...
static void emit_shared_ensure_init(Emitter *e, Node *root, const char *arrow) {
    ...
    /* BUG-973: the CHECKER's flag, which is set for every `@cond_*` in every
...
static void emit_zero_value(Emitter *e, Type *t) {
    ...
    if (inner->kind == TYPE_OPTIONAL && !is_null_sentinel(inner->optional.inner)) {
        ...
        /* BUG-951: a slice is a {ptr,len} struct too. A bare `0` here is what
...
/* BUG-994 / BUG-995 (2026-09-05): the emitter no longer synthesises any
...
static void emit_array_as_slice(Emitter *e, Node *array_expr, Type *array_type,
    ...
    emit(e, "*)");
    /* BUG-986: `func` selects the dispatch path. In an IR-rewritten expression
...
/* BUG-983: `bool` is a two-variant enum in everything but spelling, and it has
...
static void emit_enum_variant_guard_path(Emitter *e, Type *t, const char *path,
    ...
    if (k == TYPE_BOOL) {
        /* BUG-983 — see type_carries_enum_e. The variant set of `bool` is
...
/* ========================================================= * CAST EMISSION — ONE POLICY, THREE CALL SITES  (BUG-963, 2026-09-02)
 * ================================================================
 *
...
/* BUG-967: the same saturation as a C CONSTANT EXPRESSION — no statement
...
/* BUG-964 (2026-09-02): a function whose RETURN TYPE is a function pointer needs
...
static void emit_expr(Emitter *e, Node *node) {
    ...
    case NODE_IDENT: {
        /* BUG-942: inside a GLOBAL initializer a name is not a C constant
    ...
    case NODE_ORELSE: {
        ...
        if (node->orelse.fallback_is_return || node->orelse.fallback_is_break ||
            ...
            emit(e, ") { ");
            /* BUG-995: this is the AST emitter (global initialisers, spawn
    ...
    case NODE_TYPECAST: {
        /* Site 1 of 3. All cast policy lives in emit_cast_value (BUG-963). */
    ...
    case NODE_INTRINSIC: {
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            if (tgt_eff && tgt_eff->kind == TYPE_POINTER &&
                ...
                uint32_t tid = zer_pointee_tid(src_eff);   /* BUG-971 */
                ...
                uint32_t expected_tid = zer_pointee_tid(tgt_eff);   /* BUG-971 */
                if (expected_tid > 0) {
                    int tmp = e->temp_count++;
            ...
            /* BUG-971: TWO ids, because @pun has two situations.
             *
             ...
             * rejected by BH-18 #4, forging by BUG-969, everything else
            ...
            if (src_is_opaque) {
                 ...
                 * BUG-971: the DYNAMIC id here — the source's true origin is only
            ...
            /* BUG-950: synthesized by ir_lower at the last arm of an exhaustive
             ...
             * BUG-959: was `nlen >= 10`, which excludes `@atomic_or` (9 chars) —
             ...
             * BUG-984 (1zukjq): the branch also had NO trailing `else`, so the seven
             * ops the IR path handles and this one does not (xchg, nand, the five
             * *_fetch forms) emitted the EMPTY STRING; they now land on the loud
             * BUG-767 diagnostic. */
            ...
            if (is_load) {
                ...
                /* BUG-984 — see above. No AST-path handler for this atomic op. */
            ...
            if (is_timedwait && node->intrinsic.arg_count >= 3) {
                ...
                /* BUG-984: this emitted a C comment naming the intrinsic
    ...
    if (node->struct_decl.is_shared) {
        if (node->struct_decl.is_shared_rw) {
            ...
            if (st && type_dispatch_kind(st) == TYPE_STRUCT)
                needs_condvar = st->struct_type.uses_condvar;   /* BUG-973 */
...
static void emit_func_decl(Emitter *e, Node *node) {
    ...
    /* BUG-964: the prototype path had NO funcptr-return handling at all, so a
...
/* BUG-973: `register_condvar_type` / `is_condvar_type` and the
...
static void prescan_spawn_in_node(Emitter *e, Node *node) {
    ...
    case NODE_EXPR_STMT:
        /* BUG-973: the condvar prescan that lived here is GONE. It matched a
...
/* --- the shared cast emitter declared beside classify_cast (BUG-963) --- */
static void emit_cast_operand(Emitter *e, const CastOperand *op) {
    switch (op->kind) {
...
static void emit_cast_value(Emitter *e, const CastOperand *op,
    ...
    case CASTF_TO_OPAQUE: {
        uint32_t tid = zer_pointee_tid(src_eff);   /* BUG-971: the erasure id */
        emit(e, "(_zer_opaque){(void*)(");
    ...
    case CASTF_FROM_OPAQUE: {
        uint32_t expected = zer_pointee_tid(tgt_eff);   /* BUG-971 */
        if (expected > 0) {
            int tmp = e->temp_count++;
            emit(e, "({ _zer_opaque _zer_pc%d = ", tmp);
            emit_cast_operand(e, op);
    ...
    case CASTF_TO_BOOL:
        ...
        break;
    case CASTF_F2I: {
        /* BUG-967: the AST site IS the global-initializer path, where a GCC
         * statement expression is illegal C (`u32 g = (u32)1e20;` used to reach
         * GCC as `uint32_t g = ({ ... });` at file scope). Prefer the
         * constant-expression form there; it declines an operand that cannot be
         * re-evaluated (side effects / volatile), which cannot occur in a
         * constant initializer, and the statement form stays for the two
         * function-body sites where it is legal. */
...
static void emit_rewritten_node(Emitter *e, Node *node, IRFunc *func) {
    ...
    case NODE_ASSIGN: {
        ...
        /* BUG-956: a BIT-SLICE target (`r[11..8] = v` on a u12) has the uN type
    ...
    case NODE_CALL: {
        ...
        if (node->call.callee && node->call.callee->kind == NODE_FIELD &&
            ...
            /* ThreadHandle.join() → pthread_join(th, NULL).
             *
             * BUG-977: the comment here used to say "field name `join` + object
        ...
        emit(e, "(");
        /* BUG-986: argument COERCIONS, the other half of this dual dispatch.
        ...
        if (nlen == 4 && memcmp(name, "size", 4) == 0) {
            ...
            if (node->intrinsic.type_arg) {
                ...
                if (is_signed) {
                    ...
                    /* BUG-990: computed in UNSIGNED. `1LL << 63` shifts into the
            ...
            /* BUG-950: the last arm of an exhaustive enum switch used to be
            ...
            if (src_is_opaque) {
                 ...
                 * BUG-971: the DYNAMIC id — see the AST sibling. */
                uint32_t tgt_tid = tgt_tid_dyn;
                if (tgt_tid > 0) {
                    int tmp = e->temp_count++;
                    emit(e, "({ _zer_opaque _zer_pn%d = ", tmp);
            ...
            /* BUG-959: this is the IR-path TWIN of the AST-path fallback that
...
static void emit_ir_inst(Emitter *e, IRInst *inst, IRFunc *func) {
            ...
            if (inst->func_name) {
                ...
                if (callee->field.object && callee->field.object->kind == NODE_IDENT) {
                    ...
                    if (hot && type_dispatch_kind(hot) == TYPE_HANDLE) {
                        /* BUG-978: calling a function-pointer FIELD through a
    ...
    case IR_GOTO: {
        emit_indent(e);
        /* BUG-976: union switch totality guard (1zukjq's BUG-976). ir_lower.c
        ...
        if (inst->cast_type && inst->src1_local >= 0 &&
            ...
            /* BUG-983: the BOOL sibling. `bool` is a `uint8_t` in the emitted C
    ...
    case IR_DEFER_PUSH:
    case IR_DEFER_FIRE:
        /* BUG-995 (2026-09-05): MARKERS. A defer body is lowered inline at
    ...
    case IR_NOP:
        ...
        if (inst->expr) {
            if (inst->expr->kind == NODE_ASM) {
                ...
                /* BUG-995: ir_lower lowers EVERY switch to IR branches
    ...
    case IR_CAST: {
        ...
        if (inst->dest_local >= 0 && inst->cast_type) {
            ...
            /* Site 3 of 3 — see emit_cast_value (BUG-963). The operand is a
