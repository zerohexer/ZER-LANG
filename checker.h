...
typedef struct {
    ...
    struct ResourceUse {
        Symbol *sym;
        int line;
        const char *kind;   /* "arena" / "barrier" — for the diagnostic */
        const char *fix;    /* the call the user is missing */
    } *resource_uses;
    int resource_use_count;
    int resource_use_capacity;
...
void check_resource_init(Checker *c); /* keep inference: transitive escape fixpoint + deferred enforcement (after ALL bodies) */
