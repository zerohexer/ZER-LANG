...
The BUG-949 copy rule already refuses copying such a struct.

### LOW — enum with no zero variant: CARRIER storage is guarded only at the switch
BUG-950 rejects a bare `E e;` and traps at an exhaustive switch. A struct field, array element,
...
BUG-948 makes a static local shared between ISR and main require `volatile`; the "volatile but
...
`H h; h.p = &v; h.q = &x; ThreadHandle th = spawn w(h);` — BUG-957's borrow root is single-
...
## ~~OPEN — BRANCH `loving-davinci-r3an9y` (2026-09-04): 12 holes, 35 live negatives~~ — **ADOPTED WHOLE 2026-09-06 as BUG-946..957 (cherry-pick of `df4e51ee`), DO NOT REDO**

> All 35 negatives reject for their stated reason, all 12 positives run, the trap traps. Items B..I below are CLOSED; the residuals the branch itself recorded are the section just above. A's consumer-side residual (the switch-arm trap) came with the pick as BUG-950. Kept for its reproducers.
...
### ~~CLASS 3 — LOOP COUNTER PAST END / OFF BY ONE~~ — **CLOSED 2026-09-06 as BUG-958, DO NOT REDO**
...
### ~~CLASS 5 — ATOMIC CELL vs SCOPED SPAWN~~ — **CLOSED 2026-09-06 as BUG-960, DO NOT REDO**

> Same adoption. The scoped spawn now opens the atomic-cell window
> (`Checker.scoped_spawn_live`, `ff_spawn_in_func`); `.join()` closes it only when no
...
### ~~CLASS 6 — RMW SPLIT OVER TWO STATEMENTS~~ — **CLOSED 2026-09-06 as BUG-974, DO NOT REDO**
...
### ~~CLASS 7 — VIEW / STRUCT-INIT FIELD~~ — **CLOSED 2026-09-06 as BUG-979..981, DO NOT REDO**
...
### ~~CLASS 12 — FUNCPTR FACTORY THROUGH switch / do-while~~ — **CLOSED 2026-09-06 as BUG-961, DO NOT REDO**

> Cherry-picked `pstdqk` `bdac25c` whole (BUG-961..965): both `scan_returned_funcname` and
> its ISR sibling are exhaustive switches now; +4 REACH / +2 ISR cells; all 3 negatives
> reject for the stated reason. The same pick brought the spawn-arg `&x` VRP widening
> (BUG-962), the cast class-kill (BUG-963), the optional-funcptr return declarator
> (BUG-964), the bare-switch-arm shared lock (BUG-965), and two new matrices
> (`test_sharedlock_matrix.c`, `test_borrow_join_matrix.c`). Original:
...
static. That is BUG-985, fixed independently — it is a real precision win at the escape
...
Treat every row as a HYPOTHESIS and apply the MEASURE-FIRST protocol. Their lead A
(`ir_merge_states` field coverage) is already answered under NON-TEST FINDINGS (measured,
mostly refuted, `pool_name` closed); their lead E (`ir_lower_interrupt` + defers) is REFUTED,
measured; their lead G (bare switch arm lock) is CONFIRMED and fixed as BUG-965. What remains:

- **B.** `IRPathState.critical_depth` is not merged at all — `ir_ps_copy` supplies it from
  `states[first_live]`. If one predecessor is inside `@critical` and another is not, the
  `@critical` spawn/slab bans would read the wrong depth on the merged path.
- **C.** `ir_ps_copy` forces `terminated = false`, so the `first_live` search, the "all preds
  terminated ⇒ unreachable" branch and the dead-path skip in the merge never fire. Either the
  field does nothing, or it is meant to and does not. Establish which before touching it.
- **D.** Terminator/opcode enumerations that disagree: `dfs_reachable` / `cfg_reaches_fire`
  treat `IR_AWAIT` as a terminator inconsistently with the others; two sites both treat SLICE
  as reference-producing; `zercheck_ir.c` has no exhaustive opcode switch, so a new opcode
  is silently absorbed.
- **F.** `pre_lower_orelse` on `await_stmt.cond` hoists an orelse in the condition into the
  block BEFORE the await, so on resume the emitter re-reads a temp that is never recomputed —
  `await (poll() orelse false);` would latch its first value.
- **H.** `expr_mentions_name` still has the gap its twin had: `expr_mentions_global` was
  widened by BUG-856 to INTRINSIC / CALL / ORELSE / SLICE; `expr_mentions_name` (feeds
  `rmw_scan_body`) covers only IDENT / BINARY / UNARY / FIELD / INDEX / TYPECAST.
- **I.** Depth and width caps with no diagnostic on exhaustion: `resolve_write_target_global`
  bails at depth > 6 at both RMW sinks; `RMW_ALIAS_MAX 16` silently drops bindings past 16 (a
  dropped binding is a MISSED race — the unsafe direction); `Type *found[4]` caps shared types
  per statement; `Symbol.rmw_param_mask` has no conservative fallback past index 63.
- **J.** Allocator-name enumerations that will rot: `ir_classify_method_call_ex` knows none of
  the Ring methods although `IR_RING_*` exist; the cstdlib list is malloc/calloc/realloc only;
  `ThreadHandle` recognises only `join` while two diagnostics advise `detach`.
- **K.** `IR_SPAWN`, `IR_FIELD_WRITE`, `IR_INDEX_WRITE` are never emitted by lowering yet have
  full handlers in `ir_check_inst`; `is_thread_handle` is set ONLY inside the dead `IR_SPAWN`
  handler, so a fix applied to any of the three would be silently ineffective.

**Borrow-join over-rejection — the grid now EXISTS (`tests/test_borrow_join_matrix.c`, in
`make check`):** 9 hard-gated negatives pin the scoped-borrow soundness; 2 PENDING cells pin
the join-on-every-arm over-rejection with INVERTED expectation, so the relaxation cannot land
silently. The soundness obligation (five conditions, two of them still to be MEASURED — a
different handle declared inside an arm borrowing the same local, and break/continue/return/
goto against a linear tracker) is written in the matrix file's header.

## OPEN — four findings `v7pucv` MEASURED but deliberately did not change (adopted 2026-09-06 with BUG-958..960)
...
owner's call, like float→int. BUG-958 closed the sharpest edge (the off-by-N loop is now a
...
breach, and now has a second axis.** BUG-969 forged a pointer from an integer, which is
exactly what that probe exists to detect, and it reported clean. Its oracle is GCC's
`-Werror=int-conversion` over an INTEGER placed in each argument POSITION; BUG-969's route
...
non-vacuous: 15 breaches against a pre-BUG-969 build, 0 after.**
