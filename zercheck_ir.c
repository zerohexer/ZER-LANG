...
/* BUG-1011: alloc_id 0 is the "no allocation identity / untracked" SENTINEL —
...
static IRPathState ir_merge_states(IRPathState *states, int state_count) {
    ...
    for (int si = first_live + 1; si < state_count; si++) {
        ...
        for (int hi = 0; hi < result.handle_count; hi++) {
            ...
            /* BUG-1013: UNION the view sets. `view_alloc_ids` records the
...
/* BUG-1013: fill `dh`'s VIEW SET from a multi-view call result.
 *
...
static bool ir_register_alloc_result(ZerCheck *zc, IRFunc *func, IRPathState *ps,
    ...
    if (mc == IRMC_ALLOC || mc == IRMC_ALLOC_PTR) {
        ...
        if (h) {
            ...
            h->state = IR_HS_ALIVE;
            h->alloc_line = line;
            h->alloc_id = ir_alloc_id_of_local(dest);   /* BUG-1011: never 0 */
...
static void ir_check_inst(ZerCheck *zc, IRPathState *ps, IRInst *inst, IRFunc *func) {
        ...
        if (inst->expr && inst->expr->kind == NODE_ASSIGN &&
            ...
            if (rv && rv->kind == NODE_CALL && rv->call.callee &&
                ...
                if (cs && cs->returns_param_color > 0) {
                    ...
                    /* BUG-1013: the MULTI-VIEW sibling of the arm above. It was
    ...
    case IR_STRUCT_INIT_DECOMP: {
        Node *si = inst->expr;
        /* BUG-1012: J3 (above) registered the ALIAS but left this case the only
        ...
        if (si && si->kind == NODE_STRUCT_INIT && inst->dest_local >= 0 &&
            ...
            for (int i = 0; i < inst->call_arg_local_count; i++) {
                ...
                if (vloc < 0) continue;
                /* BUG-1012 (move half): a move struct handed to a designated
                 ...
                 * sink (see its comment: all thirteen must funnel there). The
                 * alias half below keeps HEAD's shape: a value with no bare
                 * handle may still be a struct CARRYING allocations (BUG-984),
                 * so there is no early `continue` here. */
...
bool zercheck_ir(ZerCheck *zc, IRFunc *func) {
    ...
    for (int bi = 0; bi < func->block_count; bi++) {
        ...
        for (int hi = 0; hi < ps->handle_count; hi++) {
            ...
            /* BUG-1014 (latent): the four report sites below index
