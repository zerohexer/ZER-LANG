...
- **Phase 8d: `emit_ir_value` DELETED, `can_lower_expr` DELETED (2026-04-16):**
  ...
  - **BUG-524:** field default accessor for pointer-result objects (17 Handle tests). **BUG-525:** emit_opt_wrap_value bypassed emit_rewritten_node. **BUG-526:** ThreadHandle.join() detection. **BUG-527:** @size named type resolution. **BUG-528:** Handle auto-deref through array index in default path (6 tests).
  - **195/195 ZER --use-ir, 786/786 rust, make check green.**
  - **`emit_expr` in `emit_ir_inst`: ZERO. 1 remains in `emit_rewritten_node`** (builtin calls: pool/slab/ring/arena). All other expression types handled directly: ident, literals, binary (opaque .ptr, optional .has_value/.value), unary, field (enum, slice, array, pointer ->, Handle auto-deref with get()->), call (ThreadHandle.join, comptime, field/index callee, coercions), typecast, struct_init, @size (type_arg + ident-arg + struct prefix), 13 intrinsics, slice, NODE_ASSIGN (simple, optional wrap, array→slice, volatile memmove, shared struct lock/unlock, bit-extract shift/mask), NODE_INDEX (.ptr for slices).
  - **Builtin extraction was VERIFIED WORKING (195/195, 0 emit_expr) but lost to accidental `git checkout`.** Implementation: `emit_builtin_inline(Emitter*, Node*, IRFunc*)` function handles all ~15 builtin methods. Key details: pool.alloc uses capacity integer (NOT `&pool.count`), Arena.over is static constructor on type, Task.new/delete use `_zer_auto_slab_TypeName`. Re-implementation needed — same pattern, ~200 lines. The function was placed before `emit_rewritten_node`, forward-declares `emit_rewritten_node` for arg emission, returns bool. NODE_CALL handler calls `emit_builtin_inline(e, node, func)` instead of `emit_expr(e, node)`.
