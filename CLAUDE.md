...
| **Cross-module C NAME ("is this generated name built the same way at the definition and at the use?")** | the emitter's definition site, the emitter's use site, and the checker's symbol registration — per feature that generates a C name | Compile a program that CROSSES a module boundary; neither side is wrong on its own. Found three in one sweep (BUG-865 spawn target unmangled, BUG-866 async internals mangled where the checker did not, BUG-867 container stamp defined unmangled and referenced mangled AND emitted once per module from a shared list). `test_modules/spawn_user.zer` + `xfeat_user.zer` are the gate; add a case when a feature grows a generated name |
| **Non-null carrier ("would auto-zero fill this with a NULL its type forbids?")** | local var-decl, global var-decl, array of either, struct field | ONE query `nonnull_zero_hole` + `check_nonnull_zero_init` called from BOTH var-decl sites. Was 3 hand-rolled copies covering 1 of 5 carriers for funcptrs; a global funcptr emitted a raw indirect call through address 0 (BUG-856). Struct field is the one carrier still open — see limitations.md |
| **Funcptr C declarator ("where does the name go in `ret (*name)(params)`?")** | bare, `distinct`, `?`, `?distinct`, `distinct ?`, and array-of-any-of-them | ONE peeler `funcptr_under` (strips every representation-preserving wrapper). Was 5 copies; the array one knew fewer wrappers than its siblings, so `?FuncPtr[N]` emitted an abstract declarator with a name glued on — not C (BUG-857) |
| **Type-or-value name in an intrinsic (`@size(x)`, `@offset(x, f)`)** | @size AST path, @size IR path, @offset AST path, @offset IR path | Resolve in the CHECKER (local scope is live there) and publish on the ident node; emitters read the typemap, never re-decide. Global-only lookup made a local SHADOWING a type name measure the TYPE — silent wrong number (BUG-858) |
...
u32 bits = reg[9..8];                               // bit extraction (also writes: reg[3..2] = 1)
