...
| **Bounded walk overflow ("what does this predicate answer when its depth guard trips?")** | every `depth > N` / `guard++ < N` in checker.c, zercheck_ir.c, emitter.c — ~30 sites | TWO shapes (compiler-internals.md "Bounded walks and the call-hop scans"). Syntax/type walks: limit ABOVE the parser bound (`ZER_EXPR_WALK_LIMIT` / `ZER_TYPE_WALK_LIMIT`) AND the trip returns the caller's REJECT answer — nine guards answered the ACCEPT side (BUG-975: a 34-deep carrier reached a spawn, a 10-deep call launder and an 11-long `orelse` chain stored `&local` in a global, a 14-level RMW was unseen at BOTH race sinks, a Handle 34 deep leaked unreported, an enum forged 11 deep ran unguarded). A predicate feeding TWO questions has NO conservative value (Z1) — unreachability only. Callee-descending walks: NEVER a depth cap — `walk_on_path` cycle cut + per-Symbol memo + loud cap (BUG-977: `fib` from a spawn target HUNG the compiler, a 35-deep chain was silently unscanned). Gate: the BUG-975..977 negatives in `tests/zer_fail/` + `tests/zer/spawn_recursive_callee_compiles.zer`; no auto-gate — grep the guards when you add one |
