...
| `sink_matrix.sh` | `SINK MATRIX CLEAN` (cell count printed by the gate — do not quote it from here) |
...
(2026-08-28).** All ten `tests/test_*_matrix.c` grids hardcoded `./zerc` in `find_zerc` and
IGNORED argv, so `./test_hw_matrix /path/to/prefix/zerc` silently graded the CURRENT
compiler and reported the same all-green for both. That turns the one step that proves a new
cell is a net rather than a script into a no-op. They now honour **`ZER_MATRIX_ZERC`**, the
same affordance `tools/sink_matrix.sh` has always had as its first argument. Build a pre-fix
compiler with `git archive <sha> | tar -x -C $S` + a single `gcc *.c`, then
