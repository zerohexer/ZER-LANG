...
/* ----------------------------------------------------------------
 * Variant 2C funcptr: *(args) -> ret  (typedef-free, ZER-native)
 *
 * Parses the type after the leading '*' has already been consumed.
 * Caller has verified next token is '('.
 *
 * Forms:
 *   *(args) -> ret    pointer to function returning ret
 *   *(args)           pointer to function returning void
 *   *()               pointer to no-arg void-returning function
 *   *(keep *T, U)     keep modifier on individual params
 *
...
static bool is_type_token(TokenType type) {
    ...
    case TOK_POOL: case TOK_RING: case TOK_ARENA: case TOK_HANDLE: case TOK_SLAB: case TOK_BARRIER: case TOK_SEMAPHORE:
...
static TypeNode *parse_type_inner(Parser *p) {
    ...
    /* *T — pointer type, OR *(args) -> ret (Variant 2C funcptr) */
    if (check(p, TOK_STAR)) {
        /* Peek: if '*' is followed by '(', it's a 2C funcptr type.
         * Otherwise it's a normal pointer type. The '*' BEFORE the '('
         * is the discriminator — no other ZER type starts with '('. */
...
/* Parse function pointer type with context-dependent ? handling.
 * BUG-420: typedef sites pass ? as return type (funcptr returning ?T).
 * All other sites unwrap ? and re-wrap as optional funcptr (nullable).
 * Caller must already have consumed '(' and '*' tokens.
 * This helper enforces the invariant in one place — 6 call sites. */
static TypeNode *parse_funcptr_with_opt(Parser *p, TypeNode *type,
                                         const char **name, size_t *name_len,
                                         bool is_typedef) {
    bool is_opt = (type->kind == TYNODE_OPTIONAL);
    TypeNode *ret = (is_opt && !is_typedef) ? type->optional.inner : type;
    TypeNode *fp = parse_func_ptr_after_ret(p, ret, name, name_len);
    if (is_opt && !is_typedef) {
        TypeNode *opt = new_type_node(p, TYNODE_OPTIONAL);
        opt->optional.inner = fp;
        fp = opt;
    }
    return fp;
}
...
static Node *parse_var_decl(Parser *p, bool is_const, bool is_static, bool is_volatile) {
    ...
    if (is_func_ptr_start(p)) {
        ...
        advance(p); /* consume '*' */
        {
            ...
            size_t fpname_len = 0;
            type = parse_funcptr_with_opt(p, type, &fpname, &fpname_len, false);
...
static Node *parse_switch_stmt(Parser *p) {
    ...
    while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF) && !p->oom) {
        ...
        if (check(p, TOK_LBRACE)) {
            ...
            /* BUG-1002 (2026-09-02): and then wrap THAT in a single-statement
             ...
             *
             * (`defer <stmt>;` builds a bare EXPR_STMT the same way. On the
             * branch that wrote this it measured unaffected, because defer
             * bodies were then replayed through the raw-AST path; once refactor L
             * lowered defer bodies into the IR the same gap opened there and was
             * closed at the lowering — `lower_body` / `lower_stmt_in_block` in
             * ir_lower.c, BUG-990 — so both non-block body positions now share
             * ONE per-statement wrapper. The shared-lock matrix's `defer-bare`
             * cell is the gate.) */
...
static Node *parse_struct_decl(Parser *p, bool is_packed) {
    ...
    while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF) && !p->oom) {
        ...
        if (is_func_ptr_start(p)) {
            ...
            size_t fname_len = 0;
            type = parse_funcptr_with_opt(p, type, &fname, &fname_len, false);
            f->type = type;
            f->name = fname;
...
static Node *parse_func_or_var(Parser *p, bool is_static) {
    ...
    if (is_func_ptr_start(p)) {
        ...
        advance(p); /* consume '*' */
        {
            const char *fpname = NULL;
            size_t fpname_len = 0;
            TypeNode *fp_type = parse_funcptr_with_opt(p, type, &fpname, &fpname_len, false);
            if (!fpname) {
        ...
        if (!check(p, TOK_RPAREN)) {
            do {
                ...
                if (is_func_ptr_start(p)) {
                    ...
                    const char *fpname = NULL;
                    size_t fpname_len = 0;
                    param->type = parse_funcptr_with_opt(p, param->type, &fpname, &fpname_len, false);
                    param->name = fpname;
...
static Node *parse_declaration(Parser *p) {
    ...
    if (match(p, TOK_DISTINCT)) {
        ...
        if (is_func_ptr_start(p)) {
            ...
            const char *fpname = NULL;
            size_t fpname_len = 0;
            type = parse_funcptr_with_opt(p, type, &fpname, &fpname_len, true);
