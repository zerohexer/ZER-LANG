...
| VRP range JOIN (merge at a control-flow join) | NODE_IF, if-else, switch-arm, for, while, do-while, goto/label, nested-if | **`tests/test_vrp_join_matrix.c` (GATE BUILT 2026-08-20 — this row used to read "NO auto-gate")**: join-kind x direction (narrow/widen) x payload (hazard/control), `make check`. Behavioural, not a diagnostic — the failure is a SILENT OOB. Verified to fire by disabling the NODE_IF range restore. The CONTROL cells pin over-rejection in the same grid, so a "fix" cannot buy soundness with a blanket guard |
