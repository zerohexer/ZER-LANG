...
typedef struct {
    ...
    int *defer_flags;
    /* BUG-995: scope depth at REGISTRATION, parallel to defer_bodies. When a
    ...
    /* BUG-994: auto-guard lowering state. critical_depth / defer_body_depth
    ...
    int cur_defer_origin;     /* BUG-995: 1-based defer index while lowering its body, else 0 */
    Node *guarded_inline[32];
    Node **guarded;
    int guarded_count;
    int guarded_cap;
    /* BUG-995: names DECLARED inside the defer body being registered. The
...
/* THE choke point every instruction goes through. BUG-994: an instruction
...
static void emit_inst(LowerCtx *ctx, IRInst inst) {
    inst.defer_origin = ctx->cur_defer_origin;   /* BUG-995 */
...
static void emit_3ac(LowerCtx *ctx, IRInst inst) {
    inst.defer_origin = ctx->cur_defer_origin;   /* BUG-995 */
    if (inst.expr) lower_auto_guards(ctx, inst.expr);   /* BUG-994 */
    ir_block_add_inst(&ctx->func->blocks[ctx->current_block], ctx->arena, inst);
}
...
static int lower_expr(LowerCtx *ctx, Node *expr) {
    ...
    /* The switch above is exhaustive and has no `default:` (deliberately — that
     * is what makes -Werror=switch catch a new NodeKind). But C does not
     * guarantee an enum-typed value is one of the enumerators, so control CAN
     * reach here on a corrupted node, and falling off the end of an int-returning
     * function yields an INDETERMINATE value — which this function's callers use
     * as a LOCAL ID and index `func->locals[]` with. GCC flagged it
     * (-Wreturn-type) and it was the only such site in the tree. -1 is the
     * defined "no value" answer every caller already handles. */
    return -1;
}
...
/* ================================================================
 * Defer bodies are LOWERED INTO THE IR at every fire site (BUG-995, 2026-09-05)
 *
...
static void rewrite_idents(LowerCtx *ctx, Node *expr) {
    ...
    switch (expr->kind) {
    case NODE_IDENT: {
        /* BUG-995: a name the defer body declares itself is not rewritten at
...
/* BUG-995: record a name the defer body declares (var-decl, for-init decl,
...
/* BUG-935: the multi-root shared-lock collector used a FIXED 16-slot array and
 * dropped everything past it SILENTLY — `add_shared_root_unique` returned at
 * `*count >= max` and `find_all_shared_roots_expr` returned at the same test.
 * MEASURED on the pre-fix compiler: a statement reading 18 roots of one
 * `shared(rw)` type emitted 15 `rdlock` calls and read s17/s18 with NO LOCK, with
...
static void srv_init(SharedRootVec *v) {
    ...
    v->count = 0;
    v->cap   = (int)(sizeof(v->inline_buf) / sizeof(v->inline_buf[0]));
}

static void srv_free(SharedRootVec *v) {
    if (v->items != v->inline_buf) free(v->items);
    v->items = v->inline_buf;
    v->count = 0;
}

/* Growth failure is the one case where dropping a root would be silent again, so
 * it is NOT ignored: the caller checks srv_overflowed() and refuses the program. */
static bool srv_push(SharedRootVec *v, Node *n) {
    if (v->count >= v->cap) {
        int ncap = v->cap * 2;
        Node **nb = (Node **)malloc((size_t)ncap * sizeof(Node *));
        if (!nb) return false;
        memcpy(nb, v->items, (size_t)v->count * sizeof(Node *));
        if (v->items != v->inline_buf) free(v->items);
        v->items = nb;
        v->cap   = ncap;
    }
...
static void lower_orelse_to_dest(LowerCtx *ctx, int dest_local, Node *orelse_node, int line) {
     ...
     * (BUG-995) firing them under the lock would also nest a second struct's
...
/* ================================================================
 * Auto-guard lowering (BUG-994, 2026-09-05)
 *
...
static void lower_stmt(LowerCtx *ctx, Node *node) {
    ...
    case NODE_IF: {
        ...
        /* BUG-946: the capture is scoped to the THEN body. It is created at the
    ...
    case NODE_FOR: {
        ...
        Node *init_root = NULL;
        SharedRootVec init_extra; srv_init(&init_extra);
    ...
    case NODE_SWITCH: {
        ...
        if (is_union) {
            ...
            sw_val_local = ptr_local;   /* BUG-976 union totality guard */
        ...
        /* BUG-976: the UNION sibling of BUG-950. A union switch with no default
        ...
        /* BUG-983: the BOOL sibling of BUG-950/914. The checker REQUIRES a bool
        ...
        for (int oi = 0; oi < node->switch_stmt.arm_count; oi++) {
            ...
            if (arm->is_default) {
                 ...
                 *
                 * On this branch the guard is BUG-950's BRANCH -> bb_trap inside
                 * the arm (see below); this comment records the hazard. */
                IRInst go = make_inst(IR_GOTO, node->loc.line);
                go.goto_block = bb_arm;
                /* The ENUM totality guard is BUG-950's IR-level BRANCH at the
                 * top of this arm (below, "re-check the value INSIDE the arm"),
                 * so it is a real CFG edge zercheck sees. The 1zukjq form
                 * (annotating this goto with cast_type for the emitter) was NOT
                 * adopted for enums — two guards for one rule; it is kept for
                 * the UNION sibling only, whose tag is not spellable in AST. */
                emit_inst(ctx, go);
            } else {
            ...
            /* BUG-950: the elided compare above assumed the switch value IS one
    ...
    case NODE_CRITICAL: {
        ...
        emit_inst(ctx, begin);
        ctx->critical_depth++;   /* BUG-994: a guard inside must trap, not return */
