...
| VRP range JOIN (merge at a control-flow join) | NODE_IF (§C#13 ✅), switch-arm, for-body, while/do-while body, do-while first-iter, goto/label, **orelse-block / if-capture / defer / @once bodies (2026-08-01 ✅ — conditionally-executed bodies whose narrowing must be DISCARDED, not joined)** | mirror `vrp_snap_take/restore/join` per node-kind (JOIN for two-armed; snapshot-then-DISCARD for a body that runs on only one path / at scope-exit / once-program-wide); a missing kind = silent OOB. NO auto-gate — checklist every control-flow kind |
