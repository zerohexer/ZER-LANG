...
| **MMIO const address ("what address does this `@inttoptr` designate?")** | the `@inttoptr` range+alignment gate, the direct `@inttoptr(...)[N]` bound, the local var-decl `mmio_bound`, the global var-decl `mmio_bound` | ONE query **`mmio_const_addr`** (BUG-992). All four used plain `eval_const_expr`, which folds a LITERAL but stops at an identifier — so `const u32 UART = 0x4000_0000;` (the only spelling real firmware uses) derived NO bound and deferred the range/alignment errors to a runtime trap at boot. Use the shared query; a raw `eval_const_expr` on an `@inttoptr` address arg is the regression. Negatives: `tests/zer_fail/mmio_const_ident_{oob_index,oob_addr,misaligned}.zer` |
...
| **Enum-forge doors** ("can this conversion produce a non-variant?") | `@bitcast`, `@truncate`, `@saturate` — and `@cast` verified NOT to be one | the three `tests/zer_trap/*_enum_forged_*.zer` PLUS the consumer-side guard: an exhaustive enum `switch` re-checks the elided last arm and traps (BUG-1022, `enum_nonvariant_trap` at both emitter paths), which covers the doors no conversion opens — auto-zero, an `@inttoptr`/`@pun` load, a struct field. Patched THREE times across three sessions before the door set was written down; each fix closed one door and left the siblings |
| **Documented examples** ("does the reference still compile — and does it still get REJECTED where it says it does?") | every ```zer block in `docs/reference.md`, in BOTH directions | `tools/audit_reference_examples.sh` + `tools/reference_example_baseline.txt` — IN `make check` (the 8th gate). Twice the docs asserted a feature the parser did not have (funcptr arrays, bit-extract through a pointer). A block illustrating a REJECTION is skipped by default; give it `<!-- audit: expect-error: <substring> -->` to assert the rejection AND its reason (BUG-994 — two `// COMPILE ERROR` container examples compiled clean). Never assert only "must fail": most such blocks are fragments that die on syntax or `undefined identifier` first, so the bare form passes vacuously — same weak-oracle shape `// expect-error:` closes for `tests/zer_fail/` |
...
| `tests/zer_fail/` | ZER negative tests (must fail to compile) | 894 | `tests/test_zer.sh` |
| `tests/zer_trap/` | compile clean, MUST trap at runtime (`// expect-trap`) | 50 | `tests/test_zer.sh` |
| `tests/zer_gaps/` | known gaps — compile-clean IS the gap (expectation INVERTED) | 24 | `tests/test_zer.sh` |
...
| `tests/test_*_matrix.c` | Exhaustive axis-crossed oracles (shape/escape/keep/cflow/conc/**view-alias**/hw/async/asm/defer-goto/**sharedlock**/**borrow-join**) | 12 grids | `make check` |
