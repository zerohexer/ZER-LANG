...
- **Phase 8c target:** Replace remaining `emit_expr` calls with `emit_ast_bridge` (same logic, different name + IR-aware). Incrementally replace bridge calls with local-ID emission. Verification: `grep "emit_expr"` in emit_ir_inst = 0.
