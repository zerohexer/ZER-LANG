...
static int lower_expr(LowerCtx *ctx, Node *expr) {
    ...
    /* Unreachable while the switch stays exhaustive (-Werror=switch guarantees
     * every NodeKind has a case), but C does not know that: without this the
     * function fell off the end and returned an INDETERMINATE int, which callers
     * use as a LOCAL ID — an out-of-range index into func->locals. A garbage
     * local id is a wild read in every downstream analysis, so return the
     * well-defined "no value" sentinel instead. */
    return -1;
}
...
/* classify_builtin_call() was DELETED 2026-08-15 (dead since the CFG migration).
 * ir_lower NEVER emits IR_POOL_ALLOC / IR_SLAB_ALLOC / IR_POOL_FREE — that is the
 * intentional architecture recorded in CLAUDE.md ("these are NEVER emitted; don't
 * 'fix' the absence"): method classification happens in zercheck_ir.c via
 * ir_classify_method_call_ex on the IR_ASSIGN / IR_CALL expression. A second,
 * unused classifier in the lowerer could only drift from the live one.
 */
